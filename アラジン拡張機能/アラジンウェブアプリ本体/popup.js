...
async function init() {
  ...
  if (keyInput) keyInput.value = stored[TTB_KEY_STORAGE] || '';
...
document.getElementById('btn-download').addEventListener('click', async () => {
  ...
  setStatus('⏳ CSV+画像保存をバックグラウンドで開始しました', '');
  ...
  try {
    ...
    if (!response?.ok) {
      throw new Error(response?.error || '保存ジョブを開始できませんでした');
    }
  } catch (error) {
    setStatus(`❌ ${error.message}`, 'error');
  }
...
document.getElementById('btn-download-images').addEventListener('click', async () => {
  if (products.length === 0) return;
  setStatus('⏳ 画像だけ保存をバックグラウンドで開始しました', '');

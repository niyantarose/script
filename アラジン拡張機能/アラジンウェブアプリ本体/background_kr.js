...
const DOWNLOAD_ROOT_FOLDER = 'aladin_korea';
...
function handleImageDownload(request, sendResponse) {
  ...
  startDownload({ url, filename, saveAs: false }, sendResponse);
}
...
function handleCsvDownload(request, sendResponse) {
  ...
  startDownload({ url: dataUrl, filename, saveAs: false }, sendResponse);
}
...
async function processSaveProductsAssetsJob(job) {
  ...
  await setBackgroundStatus(
    mode === 'images_only'
      ? '⏳ 画像だけ保存をバックグラウンドで開始しました'
      : '⏳ CSV+画像保存をバックグラウンドで開始しました',
    ''
  );
  ...
  for (const product of products) {
    ...
    const imageFolder = downloadCode;

    const rootedFolder = `${DOWNLOAD_ROOT_FOLDER}/${imageFolder}`;

    if (mode === 'csv_and_images') {
      const csvContent = buildCsvContent(product);
      const csvOk = await downloadCsvTask(csvContent, `${DOWNLOAD_ROOT_FOLDER}/${downloadCode}.csv`, false);
      if (csvOk) csvCount += 1;
    }
    ...
    if (product.cover) {
      ...
      const result = await downloadImageTask(product.cover, `${rootedFolder}/1.${ext}`, false);
    ...
    for (const url of additionalUrls) {
      ...
      const result = await downloadImageTask(url, `${rootedFolder}/${sequence}.${ext}`, false);
      if (result.ok) successImages += 1;
      sequence += 1;
    }
  }
...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  ...
  if (request.action === 'enqueueSaveProductsAssets') {
    enqueueJob({
      type: 'saveProductsAssets',
      products: Array.isArray(request.products) ? request.products : [],
      mode: request.mode
    }).then(jobId => {
      const message = request.mode === 'images_only'
        ? '⏳ 画像だけ保存をバックグラウンドで開始しました'
        : '⏳ CSV+画像保存をバックグラウンドで開始しました';
      setBackgroundStatus(message, '').catch(() => {});
      sendResponse({ ok: true, jobId });
      chrome.alarms.create(JOB_ALARM_NAME, { when: Date.now() + 100 });
    }).catch(error => {
      sendResponse({ ok: false, error: error.message || 'enqueue failed' });

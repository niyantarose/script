...
test('反映後に②を流すか聞き、受注明細の書き戻し失敗は黙って捨てない(2026-07-24)', () => {
  const src=require('fs').readFileSync('Project_24/取り置き台帳.js','utf8');
  const 反映=src.slice(src.indexOf('function 取り置き登録を反映本体_'),src.indexOf('function キャンセル戻し確認を更新('));
  assert.ok(/②引当を実行して全シートを最新に/.test(反映),'台帳だけ新しく他が古い状態を放置しない');
  assert.ok(/引当実行_本体_/.test(反映),'「はい」でその場で②を流す');
  assert.ok(反映.indexOf('取り置き台帳_保存_(plan.rows);')<反映.indexOf('②引当を実行して全シートを最新に'),
    '台帳保存より後に聞く(保存前に②を流さない)');

...
test('棚確認の条件付き書式は内訳セル3+棚確認7の固定10ルールを一括設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(calls.ranges),JSON.stringify([
    {row:4,col:1,rowCount:3,colCount:17},
    {row:4,col:9,rowCount:3,colCount:1}
  ]),'商品行全体とI列(確保内訳)の2範囲(データは4行目から)');
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(clearBackground.spec),JSON.stringify({row:4,col:1,rowCount:5,colCount:17}),'旧候補数ではなくmaxRowsまで背景を消す(データは4行目から)');
  assert.strictEqual(JSON.stringify(clearBorder.spec),JSON.stringify({row:4,col:1,rowCount:5,colCount:17}),'全管理領域の格子・太線を一括で消す');
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:4,col:1,rowCount:3,colCount:17}));
...
test('取り置き登録の書式更新は0件でも全管理領域の背景と罫線を一括で消す', () => {
  ...
  assert.strictEqual(JSON.stringify(calls[0]),JSON.stringify({type:'background',spec:{row:4,col:1,rowCount:5,colCount:17},value:null}));
  assert.strictEqual(JSON.stringify(calls[1]),JSON.stringify({type:'border',spec:{row:4,col:1,rowCount:5,colCount:17},args:[false,false,false,false,false,false]}));

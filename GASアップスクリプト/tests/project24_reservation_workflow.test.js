...
test('自動予約は永続記憶せず発売日が来れば候補に戻る', () => {
  const sources=[{状態:'予約候補',bans:new Set(['201'])}];
  const future=context.取り置き_初期候補_([
    {ban:'201',code:'AAA',sku:'AAAb',qty:1,予約:true}
  ],sources);
  const first=context.取り置き_棚確認記憶を適用_(future,{});
  assert.strictEqual(first.rows.length,0,'未来予約は現在の候補から非表示');
  assert.strictEqual(first.store[future[0].取置ID],undefined,'自動予約はstoreへ永続化しない');

  const released=context.取り置き_初期候補_([
    {ban:'201',code:'AAA',sku:'AAAb',qty:1,予約:false}
  ],sources);
  const second=context.取り置き_棚確認記憶を適用_(released,first.store);
  assert.strictEqual(JSON.stringify(second.rows.map(r=>r.受注番号)),JSON.stringify(['201']),'未来判定がfalseなら再表示');

  const manual=context.取り置き_登録シート引き継ぎ_(future,[
    {取置ID:future[0].取置ID,棚確認:'予約',現物取り置き数量:''}
  ],[]);
  const third=context.取り置き_棚確認記憶を適用_(manual,{});
  assert.strictEqual(third.store[future[0].取置ID],'予約','シートで手動選択した予約はstoreへ永続化');
});
...
test('分割明細の後続予約中と出荷GOも登録候補から除外する', () => {
  const rows=context.取り置き_初期候補_([
    {ban:'302',code:'RSV',sku:'RSVb',qty:1,ステータス:'出荷待/取寄せ'},
    {ban:'302',code:'RSV',sku:'RSVb',qty:1,ステータス:'予約中'},
    {ban:'303',code:'GO',sku:'GOb',qty:1,ステータス:'出荷待/取寄せ'},
    {ban:'303',code:'GO',sku:'GOb',qty:1,ステータス:'■出荷GO'}
  ],[{状態:'候補',bans:new Set(['302','303'])}]);
  assert.strictEqual(context.取り置き_登録絞り込み_(rows).length,0);
});
...
test('登録絞り込みは現物形跡があっても予約中・出荷GOを無条件で除外する', () => {
  const rows=[
    {取置ID:'OLD-RESERVED',受注ステータス:'予約中',旧入荷日:'2026-06-25',棚確認:'',現物取り置き数量:''},
    {取置ID:'PACKED-RESERVED',受注ステータス:'部分包装 / 予約中',旧入荷日:'',棚確認:'',現物取り置き数量:''},
    {取置ID:'PACKED-GO',受注ステータス:'部分包装 / ■出荷GO',旧入荷日:'',棚確認:'',現物取り置き数量:''},
    {取置ID:'PACKED',受注ステータス:'部分包装',旧入荷日:'',棚確認:'',現物取り置き数量:''}
  ];

  const out=context.取り置き_登録絞り込み_(rows);

  assert.strictEqual(JSON.stringify(out.map(r=>r.取置ID)),JSON.stringify(['PACKED']));
});
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(clearBackground.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:14}),'旧候補数ではなくmaxRowsまで背景を消す');
  assert.strictEqual(JSON.stringify(clearBorder.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:14}),'全管理領域の格子・太線を一括で消す');
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:2,col:1,rowCount:2,colCount:14}));
...
test('取り置き登録の書式更新は0件でも全管理領域の背景と罫線を一括で消す', () => {
  ...
  assert.strictEqual(JSON.stringify(calls[0]),JSON.stringify({type:'background',spec:{row:2,col:1,rowCount:7,colCount:14},value:null}));
  assert.strictEqual(JSON.stringify(calls[1]),JSON.stringify({type:'border',spec:{row:2,col:1,rowCount:7,colCount:14},args:[false,false,false,false,false,false]}));

...
test('棚確認の条件付き書式は対象商品行の全列へ固定5ルールを一括設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(calls.ranges),JSON.stringify([{row:2,col:1,rowCount:3,colCount:16}]));
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(clearBackground.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:16}),'旧候補数ではなくmaxRowsまで背景を消す');
  assert.strictEqual(JSON.stringify(clearBorder.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:16}),'全管理領域の格子・太線を一括で消す');
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:2,col:1,rowCount:3,colCount:16}));
...
test('取り置き登録の書式更新は0件でも全管理領域の背景と罫線を一括で消す', () => {
  ...
  assert.strictEqual(JSON.stringify(calls[0]),JSON.stringify({type:'background',spec:{row:2,col:1,rowCount:7,colCount:16},value:null}));
  assert.strictEqual(JSON.stringify(calls[1]),JSON.stringify({type:'border',spec:{row:2,col:1,rowCount:7,colCount:16},args:[false,false,false,false,false,false]}));
...
test('別ルート行はアクティブ注文だけに橙・表示専用で付き、入荷日と要対応を出す', () => {
  ...
  assert.strictEqual(tw.追加数量,'','別ルートは数量入力させない(表示専用)');

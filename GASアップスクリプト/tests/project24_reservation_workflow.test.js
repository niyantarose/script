...
test('取り置き登録の注文境界は同じ注文の最終行を全18列で返す(2026-07-23 確保済み/不足列を追加)', () => {
...
test('棚確認の条件付き書式は対象商品行の全列へ固定5ルールを一括設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(calls.ranges),JSON.stringify([{row:2,col:1,rowCount:3,colCount:18}]));
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(clearBackground.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:18}),'旧候補数ではなくmaxRowsまで背景を消す');
  assert.strictEqual(JSON.stringify(clearBorder.spec),JSON.stringify({row:2,col:1,rowCount:7,colCount:18}),'全管理領域の格子・太線を一括で消す');
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:2,col:1,rowCount:3,colCount:18}));
...
test('取り置き登録の書式更新は0件でも全管理領域の背景と罫線を一括で消す', () => {
  ...
  assert.strictEqual(JSON.stringify(calls[0]),JSON.stringify({type:'background',spec:{row:2,col:1,rowCount:7,colCount:18},value:null}));
  assert.strictEqual(JSON.stringify(calls[1]),JSON.stringify({type:'border',spec:{row:2,col:1,rowCount:7,colCount:18},args:[false,false,false,false,false,false]}));

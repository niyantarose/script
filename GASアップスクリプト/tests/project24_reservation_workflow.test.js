...
test('登録絞り込みは出荷GOと入荷証拠のない予約中だけを除外する', () => {
  const rows=[
    {取置ID:'A',受注ステータス:'処理済①',旧入荷日:'2026-06-25',棚確認:'',現物取り置き数量:''},   // 着済のはず→残す
    {取置ID:'B',受注ステータス:'予約中',旧入荷日:'2026-06-25',棚確認:'予約',現物取り置き数量:''}, // 早着の証拠(旧入荷日)→棚確認へ出す
    {取置ID:'C',受注ステータス:'出荷待/取寄せ',旧入荷日:'',棚確認:'',現物取り置き数量:''},        // 帳簿上未着でも表示
    {取置ID:'D',受注ステータス:'予約中',旧入荷日:'',棚確認:'予約',現物取り置き数量:2},            // 数量入力=現物の証拠→表示
    ...
    {取置ID:'K',受注ステータス:'予約中',旧入荷日:'',旧EMS:'',棚確認:'',現物取り置き数量:''},      // 証拠なしの予約中→落とす
    {取置ID:'L',受注ステータス:'予約中',旧入荷日:'',旧EMS:'EG1',棚確認:'',現物取り置き数量:''},   // 旧EMSも早着の証拠→表示
    {取置ID:'M',受注ステータス:'予約中',旧入荷日:'',台帳確保数:1,棚確認:'',現物取り置き数量:''}   // ④の台帳確保も証拠→表示
  ];
  const out=context.取り置き_登録絞り込み_(rows);
  assert.strictEqual(JSON.stringify(out.map(r=>r.取置ID)),JSON.stringify(['A','B','C','D','E','H','I','J','L','M']));
});

test('登録絞り込みは出荷GOを無条件で除外し、予約中は早着の証拠があれば棚確認へ出す', () => {
  const rows=[
    {取置ID:'OLD-RESERVED',受注ステータス:'予約中',旧入荷日:'2026-06-25',棚確認:'',現物取り置き数量:''},    // 早着した予約品→隠さない
    {取置ID:'PACKED-RESERVED',受注ステータス:'部分包装 / 予約中',旧入荷日:'',棚確認:'',現物取り置き数量:''}, // 証拠なし予約中→落とす
  ...
  assert.strictEqual(JSON.stringify(out.map(r=>r.取置ID)),JSON.stringify(['OLD-RESERVED','PACKED']));
});
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:2,col:1,rowCount:2,colCount:17}));

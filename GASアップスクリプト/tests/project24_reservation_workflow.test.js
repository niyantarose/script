...
test('予約判定は発売予定日が未来のときだけ自動で予約にする', () => {
  ...
  const rows=context.取り置き_初期候補_(
    [{ban:'201',code:'JMEE-SPKZ-20',sku:'JMEE-SPKZ-20-4b',qty:1,kbn:'取り寄せ',予約:true},
     ...
     {ban:'203',code:'PFOX93',sku:'PFOX93b',qty:1,kbn:'取り寄せ',予約:true,入荷日:'2026-07-11'},
     {ban:'204',code:'JMEE513',sku:'JMEE513-1b',qty:1,kbn:'取り寄せ',予約:true,ステータス:'部分包装'}],
    [{状態:'着済スタンプ(要棚確認)',bans:new Set(['201','202','203'])},
     {状態:'部分包装(要棚確認)',bans:new Set(['204'])}]);
  assert.strictEqual(rows.find(r=>r.受注番号==='201').棚確認,'予約','予約は最初から選択済み');
  assert.strictEqual(rows.find(r=>r.受注番号==='202').棚確認,'');
  assert.strictEqual(rows.find(r=>r.受注番号==='203').棚確認,'','入荷日スタンプがある行は予約より届いた形跡が勝つ(棚確認に出す)');
  assert.strictEqual(rows.find(r=>r.受注番号==='204').棚確認,'','部分包装(梱包中)は予約表記でも現物ありとして出す');
  assert.strictEqual(rows.find(r=>r.受注番号==='204').現在の状態,'部分包装(要棚確認)');
});
...
// ===== 登録絞り込み: 物がある可能性が高い行だけの最小リスト =====

test('登録絞り込みは幽霊スタンプ・未来予約・帳簿未着を落とし、入力済みは常に残す', () => {
  const rows=[
    {取置ID:'A',受注ステータス:'処理済①',旧入荷日:'2026-06-25',棚確認:'',現物取り置き数量:''},   // 着済のはず→残す
    {取置ID:'B',受注ステータス:'予約中',旧入荷日:'2026-06-25',棚確認:'予約',現物取り置き数量:''}, // 幽霊スタンプ→落とす
    {取置ID:'C',受注ステータス:'出荷待/取寄せ',旧入荷日:'',棚確認:'',現物取り置き数量:''},        // 帳簿上未着→落とす
    {取置ID:'D',受注ステータス:'予約中',旧入荷日:'',棚確認:'予約',現物取り置き数量:''},           // 未来予約→落とす
    ...
    {取置ID:'I',受注ステータス:'部分包装',旧入荷日:'',棚確認:'予約',現物取り置き数量:''}          // 部分包装は予約選択より優先して出す
  ];
  const out=context.取り置き_登録絞り込み_(rows);
  assert.strictEqual(JSON.stringify(out.map(r=>r.取置ID)),JSON.stringify(['A','E','H','I']));
});

// ===== 棚確認の判断済み(出荷済み/未着)は記憶して以後表示しない =====

test('出荷済み/未着と判断した行は非表示になり記憶される・注文が消えれば記憶も消える', () => {
  const candidates=[
  ...
  assert.strictEqual(JSON.stringify(first.rows.map(r=>r.取置ID)),JSON.stringify(['B','C']),'判断済みAは非表示');
  assert.strictEqual(first.store.A,'出荷済み','判断を記憶');
  // 次回: シート側の棚確認が空でも記憶から復元されて非表示のまま
  const second=context.取り置き_棚確認記憶を適用_(
    [{取置ID:'A',棚確認:'',現物取り置き数量:''},{取置ID:'B',棚確認:'未着',現物取り置き数量:''}],first.store);
  assert.strictEqual(second.rows.length,0,'Aは記憶で非表示・Bは今回の判断で非表示');

...
const context = {
  ...
  引当_数値変更後全同期_: () => ({success:true}), // 数量変更入口のテストでは全同期本体を別テストで検証
...
test('Yahoo戻し候補は現物ありだけを保存し確認列へ一括チェックボックスを設定する', () => {
  ...
  let savedHeaderRow, checkboxSpec;
  context.取り置き_表を保存_=(sheet,headers,rows,headerRow)=>{ saved={sheet,headers,rows}; savedHeaderRow=headerRow; };
  context.SpreadsheetApp.getActive=()=>({
    getSheetByName:()=>({
      getLastRow:()=>0, getLastColumn:()=>6, getMaxColumns:()=>6,
      getRange:(row,col,rowCount,colCount)=>{ rangeCalls++; checkboxSpec={row,col,rowCount,colCount};
        return {insertCheckboxes:()=>{ checkboxCalls++; }}; }
    })
  });
  ...
  assert.strictEqual(savedHeaderRow,3,'1〜2行目はボタン置き場・3行目が見出し(2026-07-23)');
  assert.strictEqual(rangeCalls,1);
  assert.strictEqual(checkboxCalls,1);
  assert.strictEqual(JSON.stringify(checkboxSpec),JSON.stringify({row:4,col:6,rowCount:1,colCount:1}),'チェックボックスはデータ行(4行目〜)の確認列へ');
});
...
function 便締めモック_(options){
  ...
  // Yahoo出力側は別ファイル(実GASでは同居)。typeofガードを通すためスタブを注入する
  state.exportCalls=0; state.carryOver=[];
  const 対象外set=new Set((options.在庫対象外||[]).map(c=>String(c).toUpperCase()));
  context.Yahoo変更_対象行_=(rows)=>{
    const 対象=[],除外=[];
    (rows||[]).forEach(r=>{ const code=String(r.商品コード||'').trim();
      if(対象外set.has(code.toUpperCase())) 除外.push({商品コード:code,余り数:r.余り数,EMS番号:r.EMS番号,理由:'在庫対象外(テスト)'});
      else 対象.push({商品コード:code,余り数:r.余り数,EMS番号:r.EMS番号}); });
    return {対象,除外};
  };
  ...
  context.日本在庫_戻り待ち_読む_=()=>state.carryOver.map(r=>Object.assign({},r));
  context.日本在庫_戻り待ち_保存_=rows=>{ state.carryOver=rows.map(r=>Object.assign({},r)); };
  context.Utilities=Object.assign({},context.Utilities,{formatDate:()=>'2026-07-24 18:00:00'});
...
function Yahoo確定モック_(options){
  ...
  context.引当_数値変更後全同期_=()=>{ context.Yahoo戻し候補を更新_(); return {success:true}; };
  // 確定後に「日本在庫へ移す」控えを積む(2026-07-24)。シート I/O はモックで代替する
  context.日本在庫_戻り待ち_読む_=()=>state.待ち保存.map(r=>Object.assign({},r));
  context.日本在庫_戻り待ち_保存_=rows=>{ state.待ち保存=rows.map(r=>Object.assign({},r)); };
  context.Utilities={formatDate:()=>'2026-07-24 12:00:00'};
  const prevActive=context.SpreadsheetApp.getActive;
  context.SpreadsheetApp.getActive=()=>({toast:(...a)=>state.toasts.push(a),
    getSheetByName:()=>(prevActive?null:null)});
...
test('Yahoo戻し確定は確定分を「日本在庫へ移す」控えへ積み、二度押しでも重複しない(2026-07-24)', () => {
  const state=Yahoo確定モック_();
  state.execute();
  assert.strictEqual(state.待ち保存.length,1,'日本在庫に出る戻り分として控えに積む');
  assert.strictEqual(state.待ち保存[0].処理ID,'YAHOO|RETURN|OLD1');
  assert.strictEqual(state.待ち保存[0].商品コード,'PoEm65（10116569）','箱の生コードでYahoo逆引きする');
  assert.strictEqual(state.待ち保存[0].数量,1);
  assert.strictEqual(String(state.待ち保存[0].出力日時||''),'','CSVを作るまで未出力');
  state.execute(); // 同じ確定をもう一度
  assert.strictEqual(state.待ち保存.length,1,'同じ戻しを二度積まない(二重加算防止)');
  assert.ok(state.toasts.length>0,'日本在庫へ移した旨を知らせる');
});
...
test('①CSV取込は保存完了後に全シート同期を1回実行する', () => {
  const source=fs.readFileSync('Project_24/引当.js','utf8');
  const body=source.slice(source.indexOf('function 取込_実行_'),source.indexOf('function 仕分け証跡キー_'));
  assert.strictEqual((body.match(/引当_数値変更後全同期_\s*\(/g)||[]).length,1);
  assert.ok(body.indexOf('取り置き_CSV遷移を反映_')<body.indexOf('引当_数値変更後全同期_'),
    '受注明細と台帳遷移の保存後に全同期する');
});
...
test('棚確認の条件付き書式は内訳セル3+棚確認7の固定10ルールを一括設定する', () => {
  ...
  assert.strictEqual(JSON.stringify(calls.ranges),JSON.stringify([
    {row:4,col:1,rowCount:3,colCount:18},
    {row:4,col:10,rowCount:3,colCount:1}
  ]),'商品行全体とJ列(確保内訳)の2範囲(データは4行目から)');
  assert.strictEqual(calls.saved.length,1,'10ルールを1回で保存');
  assert.strictEqual(calls.saved[0].length,10,'内訳3+棚確認7');
  assert.ok(calls.saved[0].slice(0,3).every(rule=>rule.ranges[0]===calls.ranges[1]),'先頭3つは確保内訳セルだけを対象(行色より優先)');
  assert.ok(calls.saved[0].slice(3).every(rule=>rule.ranges[0]===calls.ranges[0]),'棚確認ルールは商品行全体を対象');
  assert.strictEqual(calls.saved[0][3+4].文字色,'#990000','出荷済みだけ濃い赤文字');
  assert.strictEqual(calls.saved[0][3+4].太字,true,'出荷済みだけ太字');
...
test('取り置き登録の書式更新は候補縮小時に全管理領域を消してから現在行だけへ再設定する', () => {
  ...
  const sheet={
    getMaxRows:()=>8,
    getLastRow:()=>5, // データは4行目から(1〜2行目=ボタン置き場、3行目=見出し)
    getLastColumn:()=>18,
    getRange:(row,col,rowCount,colCount)=>{
  ...
  assert.strictEqual(JSON.stringify(clearBackground.spec),JSON.stringify({row:4,col:1,rowCount:5,colCount:18}),'旧候補数ではなくmaxRowsまで背景を消す(データは4行目から)');
  assert.strictEqual(JSON.stringify(clearBorder.spec),JSON.stringify({row:4,col:1,rowCount:5,colCount:18}),'全管理領域の格子・太線を一括で消す');
  ...
  assert.strictEqual(JSON.stringify(backgrounds.spec),JSON.stringify({row:4,col:1,rowCount:3,colCount:18}));
  ...
  assert.strictEqual(calls.find(call=>call.type==='conditionalRules').rules.length,10);
});
...
test('取り置き登録の書式更新は0件でも全管理領域の背景と罫線を一括で消す', () => {
  ...
  assert.strictEqual(JSON.stringify(calls[0]),JSON.stringify({type:'background',spec:{row:4,col:1,rowCount:5,colCount:18},value:null}));
  assert.strictEqual(JSON.stringify(calls[1]),JSON.stringify({type:'border',spec:{row:4,col:1,rowCount:5,colCount:18},args:[false,false,false,false,false,false]}));
...
test('確保内訳表記: 自動確保は元EMS番号(どの箱か)ごとに出し、先行・現物・棚を区別する', () => {
  ...
  assert.strictEqual(context.取り置き_確保内訳表記_(rows,2),
    '自動8(EG050152967KR)+先行2(EG049882819KR)+現物3','棚登録は現物へ合算(どちらも俺が数えた現物)');
  assert.strictEqual(context.取り置き_確保内訳表記_([],1),'現物1');
...
test('確保表示整合: 全数確保の行は棚確認へ「部分在庫」を自動表示し、矛盾した「未着」を直す', () => {
  const rows=[
    {取置ID:'A',確保済み:1,不足:0,棚確認:'未着'},    // 全数確保なのに未着 → 過去部分在庫
    {取置ID:'B',確保済み:1,不足:1,棚確認:'未着'},    // 一部確保でもステータスを付ける(残りは不足列で見る)
    {取置ID:'C',確保済み:2,不足:0,棚確認:''},        // 全数確保の空欄 → 過去部分在庫を自動表示
  ...
  assert.strictEqual(out[0].棚確認,'過去部分在庫');
  assert.strictEqual(out[1].棚確認,'過去部分在庫','一部確保(棚1あり等)も空欄にせずステータスを付ける');
  assert.strictEqual(out[2].棚確認,'過去部分在庫');
...
test('確保表示整合: 部分在庫は確保時期で3種類に貼り分け、日をまたげば当日→部分在庫へ戻る', () => {
  const out=context.取り置き_確保表示整合_([
    ...
    {確保済み:1,不足:0,棚確認:'当日部分在庫',確保時期:'過去'},     // 日またぎ → 過去部分在庫(緑)へ落ち着く
    {確保済み:1,不足:0,棚確認:'部分在庫',確保時期:'当日'},         // 旧名の部分在庫も当日なら貼り替え
    {確保済み:1,不足:1,棚確認:'当日部分在庫',確保時期:'当日'},     // 一部確保でも時期ステータスを維持
  ...
  assert.strictEqual(out[4].棚確認,'当日部分在庫');
...
test('別ルート行の確保済み/不足: 棚登録と入荷日方式は大きい方で実効確保(足して二重計上しない)', () => {
  ...
  assert.strictEqual(a.確保内訳,'入荷日3・現物1','内訳は両方式を・区切りで列挙(足し算ではない)');
  ...
  assert.strictEqual(b.確保内訳,'現物1','自分の棚登録だと分かる内訳');
});
...
// ===== 2026-07-23 分割出荷: 行の出荷日で「もう送った分」を需要から外す(持ち戻り注文の二重確保防止) =====

test('CSV行オブジェクト: 出荷日/出荷日(複数時には配送先毎)から行出荷済みを立てる', () => {
  const header=['受注番号','受注ステータス','商品コード','商品SKU','個数','出荷日','出荷日(複数時には配送先毎)'];
  const out=context.CSV行を受注行オブジェクトへ_(header,[
    ['niyantarose-10117477','持ち戻り/問合せ中','MRBLUE41','MRBLUE41b',2,'','2026/07/14'],
    ['niyantarose-10117477','持ち戻り/問合せ中','MRBLUE41','MRBLUE41b',1,'','']
  ]);
  assert.strictEqual(out[0].行出荷済み,true,'W列に出荷日がある分割は出荷済み');
  assert.strictEqual(out[1].行出荷済み,false,'出荷日が空の分割は生きた需要');
});

test('CSV遷移: 分割出荷済みは需要から外れ、超過確保だけ棚戻し待ちへ(持ち戻り注文の二重確保防止)', () => {
  const now='2026-07-23T19:00:00';
  const ledger=[
    {取置ID:'EMS|EG050152967KR|MRBLUE41||10117477|MRBLUE41|MRBLUE41B',状態:'取り置き中',受注番号:'10117477',
     商品コード:'MRBLUE41',SKU:'MRBLUE41b',取り置き数量:3,取置元種別:'EMS',元EMS番号:'EG050152967KR',
     引当段階:'到着済',登録日時:'2026/07/23','終了理由・メモ':''}
  ];
  const plan=context.取り置き_CSV遷移計画_([
    {受注番号:'10117477',受注ステータス:'持ち戻り/問合せ中',商品コード:'MRBLUE41',SKU:'MRBLUE41b',個数:2,行出荷済み:true},
    {受注番号:'10117477',受注ステータス:'持ち戻り/問合せ中',商品コード:'MRBLUE41',SKU:'MRBLUE41b',個数:1,行出荷済み:false}
  ],ledger,now);
  assert.strictEqual(plan.errors.length,0,'発送済みステータス競合にしない(注文は継続)');
  const active=plan.rows.filter(r=>r.状態==='取り置き中');
  const returns=plan.rows.filter(r=>r.状態==='キャンセル戻し');
  assert.strictEqual(active.length,1);
  assert.strictEqual(active[0].取り置き数量,1,'待っている1個分の確保だけ残る');
  assert.strictEqual(returns.reduce((n,r)=>n+r.取り置き数量,0),2,'出荷済み分割の2個分は棚戻し待ちへ(箱の中のまま=棚へ戻したで処理)');
});

test('CSV遷移: 全分割が出荷済みでもキャンセル扱いにせず、確保の全量を棚戻し待ちへ', () => {
  const ledger=[
    {取置ID:'EMS|EG1|AAA||903|AAA|AAAB',状態:'取り置き中',受注番号:'903',商品コード:'AAA',SKU:'AAAb',
     取り置き数量:1,取置元種別:'EMS',元EMS番号:'EG1',引当段階:'到着済',登録日時:'2026/07/23','終了理由・メモ':''}
  ];
  const plan=context.取り置き_CSV遷移計画_([
    {受注番号:'903',受注ステータス:'持ち戻り/問合せ中',商品コード:'AAA',SKU:'AAAb',個数:1,行出荷済み:true}
  ],ledger,'2026-07-23T19:00:00');
  assert.strictEqual(plan.errors.length,0);
  assert.strictEqual(plan.rows.filter(r=>r.状態==='取り置き中').length,0,'需要0なので確保は残らない');
  assert.strictEqual(plan.rows.filter(r=>r.状態==='キャンセル戻し').length,1,'発送済み遷移ではなく棚戻し確認へ');
});
...
// ===== 2026-07-23 レビュー確定: 自動「未着」ラベルが人の入力と現物確認を塞がないこと =====

test('反映: 自動で貼られた「未着」でも棚で見つけた追加数量は登録できる(人の入力が機械ラベルより強い)', () => {
  const plan=context.取り置き_統合反映計画_([
    {取置ID:'INIT|301|AAA|AAAB',受注番号:'301',商品コード:'AAA',SKU:'AAAb',注文数量:1,
     追加数量:1,マイナス数量:'',棚確認:'未着',メモ:''}
  ],[],'2026-07-23T21:00:00');
  assert.strictEqual(plan.errors.length,0,'未着ラベルで弾かない');
  const row=plan.rows.find(r=>r.取置ID==='INIT|301|AAA|AAAB');
  assert.ok(row && row.取り置き数量===1,'棚で見つけた現物が台帳へ載る');
  const 出荷済み=context.取り置き_統合反映計画_([
    {取置ID:'INIT|302|BBB|BBBB',受注番号:'302',商品コード:'BBB',SKU:'BBBb',注文数量:1,
     追加数量:1,マイナス数量:'',棚確認:'出荷済み',メモ:''}
  ],[],'2026-07-23T21:00:00');
  assert.ok(出荷済み.errors.some(e=>/出荷済み/.test(e)),'人が選んだ出荷済み・予約は従来どおり止める');
});

test('確保表示整合: 自動「未着」は到着証拠が付いたら剥がれて要棚確認へ戻る(焼き付かない)', () => {
  const day1=context.取り置き_確保表示整合_([{確保済み:0,不足:1,棚確認:'',旧入荷日:'',旧EMS:''}]);
  assert.strictEqual(day1[0].棚確認,'未着','証拠が無いうちは未着');
  const day2=context.取り置き_確保表示整合_([{確保済み:0,不足:1,棚確認:'未着',旧入荷日:'2026-07-24',旧EMS:''}]);
  assert.strictEqual(day2[0].棚確認,'','入荷日が付いたら空へ戻す');
  assert.strictEqual(context.取り置き_棚確認判定_(day2[0]),'要棚確認','棚を見に行く合図が復活する');
  const 手動=context.取り置き_確保表示整合_([{確保済み:0,不足:1,棚確認:'発送待ち',旧入荷日:'2026-07-24'}]);
  assert.strictEqual(手動[0].棚確認,'発送待ち','人の判断は上書きしない');
});

test('反映: 入力エラーの行番号が実際のシート行(見出し3行目・データ4行目〜)を指す', () => {
  const plan=context.取り置き_統合反映計画_([
    {取置ID:'INIT|401|AAA|AAAB',受注番号:'401',商品コード:'AAA',SKU:'AAAb',注文数量:1,追加数量:'',マイナス数量:'',棚確認:'',メモ:''},
    {取置ID:'INIT|402|BBB|BBBB',受注番号:'402',商品コード:'BBB',SKU:'BBBb',注文数量:1,追加数量:'x',マイナス数量:'',棚確認:'',メモ:''}
  ],[],'2026-07-23T21:00:00');
  assert.ok(plan.errors.some(e=>/初期登録5行/.test(e)),'2件目(index=1)はシート5行目: '+plan.errors.join(' / '));
});

test('P列計画: 分割出荷済み(出荷日あり)の行を需要から外すガードが入っている(②と同じ判定)', () => {
  const src=require('fs').readFileSync('Project_24/P列自動記入.js','utf8');
  assert.ok(/引当_行出荷済み_\(row,M\)/.test(src) && /continue/.test(src),
...
test('空き在庫列: 見出しは不足の隣にあり、確保内訳より前に並ぶ(2026-07-24)', () => {
  const src=require('fs').readFileSync('Project_24/取り置き台帳.js','utf8');
  const line=src.split(String.fromCharCode(10)).find(l=>l.indexOf('初期HDR:[')>=0)||'';
  const hdr=(line.match(/'[^']+'/g)||[]).map(v=>v.slice(1,-1));
  assert.strictEqual(hdr[hdr.indexOf('不足')+1],'空き在庫','不足のすぐ右で対比できる');
  assert.ok(hdr.indexOf('空き在庫')<hdr.indexOf('確保内訳'));
...
test('反映本体は台帳保存の直後・再生成の前に入力欄を消す', () => {
  ...
  const 再生成=src.indexOf("引当_数値変更後全同期_({理由:'取り置き登録反映'});");
  assert.ok(保存>=0 && 消去>保存,'台帳保存に成功してから消す(失敗時は入力を残す)');
  assert.ok(再生成>消去,'再生成より前に消さないとシートから復元されてしまう');
});

test('列幅は更新で一切触らない(手動調整を常に尊重 2026-07-24)', () => {
  const src=require('fs').readFileSync('Project_24/取り置き台帳.js','utf8');
  const 作成本体=src.slice(src.indexOf('function 取り置き初期登録を作成本体_'),
    src.indexOf('function 取り置き登録を反映('));
  assert.strictEqual(/setColumnWidth/.test(作成本体),false,'更新処理から列幅設定を無くす');
  assert.strictEqual(/setRowHeight/.test(作成本体),false,'行の高さも触らない');
  assert.ok(/function 取り置き登録の列幅を既定に戻す/.test(src),'戻したい時はメニューから明示的に');
});

test('見出しの書式は見出しの中身が変わった時だけ当てる(手動の書式が毎回戻らない)', () => {
  const src=require('fs').readFileSync('Project_24/取り置き台帳.js','utf8');
  const 保存=src.slice(src.indexOf('function 取り置き_表を保存_'),src.indexOf('function 取り置き台帳_読む_'));
  assert.ok(/見出し変更/.test(保存) && /if\(見出し変更\)/.test(保存),'無条件のsetBackgroundを残さない');
  assert.strictEqual(/setValues\(\[headers\]\)\.setFontWeight/.test(保存),false);
});

test('反映したら必ず②で再引当し、受注明細の書き戻し失敗は黙って捨てない(2026-07-24)', () => {
  const src=require('fs').readFileSync('Project_24/取り置き台帳.js','utf8');
  const 反映=src.slice(src.indexOf('function 取り置き登録を反映本体_'),src.indexOf('function キャンセル戻し確認を更新('));
  assert.ok(/引当_数値変更後全同期_\(\{理由:'取り置き登録反映'\}\)/.test(反映),'反映したら必ず中央同期で②を実行する(聞かない)');
  assert.strictEqual(/②引当を実行して全シートを最新にしますか/.test(反映),false,'②を流すかの確認は廃止(必ず流す)');
  assert.ok(/棚に現物はありますか/.test(反映),'マイナスの現物確認は残す');
  assert.ok(反映.indexOf('取り置き台帳_保存_(plan.rows);')<反映.indexOf('引当_数値変更後全同期_'),
    '台帳を保存してから再引当する(保存前に②を流さない)');
  assert.ok(反映.indexOf('取り置き_反映済み入力を消す_')<反映.indexOf('引当_数値変更後全同期_'),
    '入力を消してから②(②が取り置き登録を作り直すため)');
  const 書き戻し=src.slice(src.indexOf('受注明細_確保列を書く_(candidates)'));
  assert.strictEqual(/^\s*catch\(e\)\{\}/m.test(書き戻し.slice(0,200)),false,'空catchで握り潰さない');
  assert.ok(/受注明細の確保済み\/不足\/確保内訳を更新できませんでした/.test(src),'失敗をトーストで知らせる');
});

// ===== 2026-07-24 レビュー確定: Yahooへ入らない余りを移動台帳へ記録しない =====

test('便締め: 在庫対象外(★コピペ等)の余りはYahoo移動台帳へ記録しない', () => {
  const state=便締めモック_({jpRows:[['到着済','2026-07-12','AAA',2,'EG1'],['到着済','2026-07-12','★コピペ',5,'EG1']],
    在庫対象外:['★コピペ']});
  state.execute();
  const alertText=state.alerts.map(args=>args.join(String.fromCharCode(10))).join(String.fromCharCode(10));
  assert.ok(alertText.includes('移動記録しません'),'理由付きで知らせる');
  assert.strictEqual(state.savedMoves.length,1,'★コピペは移動台帳に載らない');
  assert.strictEqual(state.savedMoves[0].処理ID,'YAHOO|EMS|EG1|AAA');
  assert.strictEqual(state.carryOver.length,0,'在庫対象外はCSVに永久に出ないので持ち越さない');
});

test('便締め: 要確認(Yahoo未出品)も記録せず、戻り待ちへ持ち越して追えるようにする', () => {
  const state=便締めモック_({jpRows:[['到着済','2026-07-12','AAA',2,'EG1'],['到着済','2026-07-12','MRBLUE41',3,'EG1']],
    要確認:['MRBLUE41']});
  state.execute();
  assert.strictEqual(state.savedMoves.length,1,'Yahooへ入っていない分は移動済みにしない');
  assert.strictEqual(state.savedMoves[0].処理ID,'YAHOO|EMS|EG1|AAA');
  assert.strictEqual(state.carryOver.length,1,'締め後も日本在庫の戻り行として残す');
  assert.strictEqual(state.carryOver[0].商品コード,'MRBLUE41');
  assert.strictEqual(state.carryOver[0].数量,3);
  assert.strictEqual(String(state.carryOver[0].出力日時||''),'','Yahoo出品後にCSVへ出すまで未出力');
});

test('便締め: 要確認判定は保存記録ではなくその場の計算を使う(古い記録で素通りしない)', () => {
  const src=require('fs').readFileSync('Project_24/引当履歴.js','utf8');
  const 締め=src.slice(src.indexOf('function 到着済を在庫反映済みへ本体_'));
  assert.ok(/Yahoo変更_要確認コード_\(出力対象\)/.test(締め),'その場で計算する');
  assert.strictEqual(/記録\.要確認コード/.test(締め),false,'保存済み記録の要確認コードには依存しない');
});

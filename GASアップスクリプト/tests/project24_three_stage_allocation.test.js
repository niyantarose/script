...
test('現物1到着2の目標は既存現物を差し引き、再実行は冪等で目標超過は不変',()=>{
  const ledger=[active('P',STAGE.PHYSICAL,1,{元EMS番号:'EMS-1',供給処理:'EMS控除'}),active('A',STAGE.ARRIVED,2,{元EMS番号:'EMS-1'})],input=n=>[{受注番号:'注文14',商品コード:'MRBLUE41',SKU:'MRBLUE41b',現物取り置き数量:n}],sup=[{ems:'EMS-1',code:'MRBLUE41',qty:3,arrival:'2026-07-20'}];
  const two=context.取り置き_現物確認変換計画_(input(2),ledger,sup,new Date('2026-07-22'));
  assert.deepStrictEqual(JSON.parse(JSON.stringify(two.rows.map(r=>[r.引当段階,r.取り置き数量]).sort())),[['到着済',1],['現物確認済み',1],['現物確認済み',1]]);
  assert.deepStrictEqual(JSON.parse(JSON.stringify(context.取り置き_現物確認変換計画_(input(2),two.rows,sup,new Date()).rows)),JSON.parse(JSON.stringify(two.rows)));
...
test('既存供給使用を引いたFIFOと衝突しない分割IDを使う',()=>{
  ...
  assert.strictEqual(p.rows.find(r=>r.取置ID!=='USED'&&r.引当段階==='現物確認済み').供給控除EMS,'NEW');assert.strictEqual(new Set(p.rows.map(r=>r.取置ID)).size,p.rows.length);
});
...
test('実例10117608は現物6と先行8を二重計上しない',()=>{
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(result)),{
    現物確認済み数量:6,到着済引当数量:0,先行引当数量:8,合計確保数量:14,要移行数量:0,行内訳:[
...
test('旧開始前在庫は要移行として分離しつつ確保数には数える(④の二重割当防止)',()=>{
  ...
  const key=context.取り置き_行キー_(row);
  assert.strictEqual(result.activeByKey[key]||0,1,'残必要の計算からは消さない');
  assert.strictEqual(result.要移行行.length,1);
  assert.strictEqual((result.byKey[key]||{}).要移行数量||0,1,'段階とは分離して持つ');
  assert.strictEqual((result.byKey[key]||{}).現物確認済み数量||0,0,'現物段階へは混ぜない');
  // 取り置き登録画面の台帳確保(=④確保分)には開始前在庫を含めない従来仕様を維持
  assert.strictEqual(context.取り置き_台帳確保集計_([row])[key]||0,0);
});

test('EMS状態: 未着だけ先行、在庫反映済み・不明・空は到着済(旧箱=物理側へ倒す)',()=>{
  assert.strictEqual(context.取り置き_EMS状態_({A:'未着'},'A'),STAGE.PLANNED);
  assert.strictEqual(context.取り置き_EMS状態_({B:'到着済'},'B'),STAGE.ARRIVED);
  assert.strictEqual(context.取り置き_EMS状態_({C:'在庫反映済み'},'C'),STAGE.ARRIVED);
  assert.strictEqual(context.取り置き_EMS状態_({},'UNKNOWN'),STAGE.ARRIVED);
});

test('④の新規台帳行は引当段階=到着済を持つ(④の供給は到着済箱のみ)',()=>{
  const row=context.取り置き_新規行_({ban:'901',code:'MRBLUE41',sku:'MRBLUE41b'},1,'EMS','EMS-9','','MRBLUE41');
  assert.strictEqual(row.引当段階,STAGE.ARRIVED);
});

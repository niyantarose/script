...
test('今回行判定_: 台帳確保済み(取り置き中)は再実行でも出せる扱いを維持する', () => {
  assert.strictEqual(context.今回行判定_({alloc:0,取り置き中数量:1}), true, '④を回し直しても出荷可能から落ちない');
  assert.strictEqual(context.今回行判定_({alloc:0,取り置き中数量:0}), false);
});

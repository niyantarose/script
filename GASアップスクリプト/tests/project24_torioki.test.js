...
test('表示列を再作成した後の列マップで発送済み分割行を判定する', () => {
  ...
  try {
    ...
    context.受注明細_確保列を用意_ = () => ({確保済み:5,不足:6,確保内訳:7});
    context.EMS番号列を用意_ = () => 10;
    ...
    assert.deepStrictEqual(writes[5], [[''],[1]]);
    assert.deepStrictEqual(writes[6], [[''],[0]]);
    assert.deepStrictEqual(writes[10], [[''],['EG050152967KR']]);
  } finally {

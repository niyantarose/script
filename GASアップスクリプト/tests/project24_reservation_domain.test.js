...
test('取り置き中の有効な三段階行だけが注文必要数を減らす', () => {
  ...
  // 旧開始前在庫は要移行だが「確保」としては数える(除外すると④が同じ注文へ二重割当する。2026-07-22)
  assert.strictEqual(summary.activeByKey[key]||0, 2, '取り置き中の開始前在庫2だけ。発送済み・手動解除は数えない');
...
test('開始前在庫が確保している注文へ④は重ねて割り当てない(供給は余りへ)', () => {
  // 2026-07-22契約変更: 旧開始前在庫は移行完了まで確保として尊重する。
  // 除外してEMSを重ねると同じ注文に現物2個(棚+箱)が付く二重確保になる。
  ...
  assert.strictEqual(result.newRows.length,0,'需要は開始前在庫で充足済み');
  assert.strictEqual(result.surplus.reduce((n,s)=>n+(Number(s.qty)||0),0),5,'箱の5個は余りへ');
});
...
test('P列確定: 同一商品の分割行注文は複数行に跨って確定を満たす(実例10117725 MEDAFB01W)', () => {
  // 2026-07-21実例: 2個+1個の分割2行に対し、1個×3箱がP列で名指し。従来は最初の行しか
  // 見つけず3個目の確定が「指定1/割当0」で④全体を中止させた
  const result=context.取り置き_割当計算_({
    orders:[
      {ban:'10117725',code:'MEDAFB01W',sku:'MEDAFB01Wb',qty:2,sortKey:1,i:0,keys:['MEDAFB01W']},
      {ban:'10117725',code:'MEDAFB01W',sku:'MEDAFB01Wb',qty:1,sortKey:1,i:1,keys:['MEDAFB01W']}
    ],
    ledger:[],movements:[],
    supplies:[
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',qty:1,arrival:'2026-07-21'},
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',qty:1,arrival:'2026-07-21'},
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',qty:1,arrival:'2026-07-21'}
    ],
    explicit:[
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',ban:'10117725',qty:1},
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',ban:'10117725',qty:1},
      {ems:'EG050152967KR',code:'MEDAFB01W',sourceCode:'MEDAFB01W',ban:'10117725',qty:1}
    ]
  });
  assert.strictEqual(JSON.stringify(result.errors),'[]');
  assert.strictEqual(result.newRows.reduce((n,r)=>n+(Number(r.取り置き数量)||0),0),3);
  assert.strictEqual(JSON.stringify(result.orders.map(o=>o.need)),JSON.stringify([0,0]));
});
...
// ===== 旧開始前在庫は要移行として④を止めず警告する =====

test('注文が消えた旧開始前在庫は中止せず要移行を警告する(④を完走)', () => {
  const result=context.取り置き_割当計算_({
  ...
  assert.ok(result.warnings.some(w=>/BEE48/.test(w)&&/要移行/.test(w)));
});

test('旧開始前在庫が注文数を超過しても要移行を警告して通常引当へ混ぜない', () => {
  const result=context.取り置き_割当計算_({
  ...
  assert.ok(result.warnings.some(w=>/AAA/.test(w)&&/要移行/.test(w)));
});
...
test('行出荷済み: 出荷日(U)か出荷日(複数時には配送先毎)(W)に値があれば分割出荷済み', () => {
  const M={出荷日:5,出荷日毎:7};
  assert.strictEqual(context.引当_行出荷済み_(['','','','','','2026/07/14','',''],M),true,'U列');
  assert.strictEqual(context.引当_行出荷済み_(['','','','','','','','2026/07/14'],M),true,'W列(分割ごと)');
  assert.strictEqual(context.引当_行出荷済み_(['','','','','','','',''],M),false);
  assert.strictEqual(context.引当_行出荷済み_([''],{出荷日:-1,出荷日毎:-1}),false,'列が無いシートでは常にfalse(従来動作)');
});
...
// ===== 2026-07-23 レビュー確定: 元EMS番号を持たない戻し(棚登録のマイナス解除)も再引当に乗せる =====

test('集計: 元EMS番号が空でも「現物あり」の戻しは confirmedReturns に載る(Yahooへ流す前に②が再引当できる)', () => {
  const rows=[
    {取置ID:'SHELF|棚外し',状態:'キャンセル戻し',受注番号:'101',商品コード:'AAA',SKU:'AAAb',
     取り置き数量:2,取置元種別:'開始前在庫',元EMS番号:'',戻し処理結果:'現物あり'},
    {取置ID:'BOX',状態:'キャンセル戻し',受注番号:'102',商品コード:'BBB',SKU:'BBBb',
     取り置き数量:1,取置元種別:'EMS',元EMS番号:'EG1',戻し処理結果:'現物あり'},
    {取置ID:'NG',状態:'キャンセル戻し',受注番号:'103',商品コード:'CCC',SKU:'CCCb',
     取り置き数量:1,取置元種別:'開始前在庫',元EMS番号:'',戻し処理結果:'在庫なし'}
  ];
  const s=context.取り置き_集計_(rows,[]);
  const ids=s.confirmedReturns.map(r=>String(r.取置ID)).sort();
  assert.strictEqual(JSON.stringify(ids),JSON.stringify(['BOX','SHELF|棚外し']),'EMS番号の有無に関わらず現物ありだけを拾う');
  assert.strictEqual(s.usageBySupply['|AAA'],undefined,'EMS供給の無い棚現物は供給使用に混ぜない');
});

test('割当計算: 棚登録をマイナス解除した現物が、同じ商品を待つ注文へ再引当される(売り越し防止)', () => {
  const result=context.取り置き_割当計算_({
    orders:[{ban:'202',code:'AAA',sku:'AAAb',qty:2,sortKey:202,i:0,keys:['AAA']}],
    ledger:[{取置ID:'INIT|101|AAA|AAAB|棚外し',状態:'キャンセル戻し',受注番号:'101',商品コード:'AAA',SKU:'AAAb',
      取り置き数量:2,取置元種別:'開始前在庫',元EMS番号:'',戻し処理結果:'現物あり',登録日時:'2026-07-23'}],
    movements:[],supplies:[],explicit:[]
  });
  assert.strictEqual(result.errors.length,0);
  const 再引当=result.newRows.filter(r=>r.取置元種別==='キャンセル再引当');
  assert.strictEqual(再引当.length,1,'棚の現物が待っている注文へ回る');
  assert.strictEqual(再引当[0].受注番号,'202');
  assert.strictEqual(再引当[0].取り置き数量,2);
  const upd=result.returnUpdates.find(u=>u.取置ID==='INIT|101|AAA|AAAB|棚外し');
  assert.ok(upd && upd.戻し処理結果==='再引当済み','再引当されたらYahoo戻し候補から自動で外れる');
});

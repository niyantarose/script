...
test('取り置き中の有効な三段階行だけが注文必要数を減らす', () => {
  ...
  assert.strictEqual(summary.activeByKey[key]||0, 0, '旧開始前在庫は要移行として通常集計から除外する');
  assert.strictEqual(context.取り置き_今回必要数_({ban:'101',code:'AAA-1',sku:'AAA-1b',qty:3}, summary), 3);
});
...
test('旧開始前取り置き5商品は要移行としてEMS引当を妨げず、新規行は開始前在庫にしない', () => {
  ...
  assert.strictEqual(result.newRows.length,5);
  assert.strictEqual(result.surplus.length,0);
  result.newRows.forEach(row=>assert.notStrictEqual(row.取置元種別,'開始前在庫'));
});

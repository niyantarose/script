...
test('供給段階: 先行はユーザーが明示した箱だけ・未着は自動で拾わない', () => {
  assert.strictEqual(context.全件再計算_供給段階_('到着済'), '到着済');
  assert.strictEqual(context.全件再計算_供給段階_('在庫反映済み'), '過去締め済み');
  assert.strictEqual(context.全件再計算_供給段階_('先行'), '先行');
  assert.strictEqual(context.全件再計算_供給段階_('未着'), '未着', '未着のままの箱は計算対象外');
  assert.strictEqual(context.全件再計算_供給段階_(''), '未着');
});

test('三段階: 状態「未着」のままの箱は需要があっても一切割り当てない(自分で選ぶ)', () => {
  const result = context.全件再計算_再構築_({
    supplies: [{ems:'EMS-NOOPT',code:'TEST-NO1',qty:2,arrival:'2026-07-23',row:2,status:'未着'}],
    shipped: [],
    currentOrders: [{ban:'910',sku:'TEST-NO1b',code:'TEST-NO1',qty:1,orderDate:'2026-07-01',row:3,route:'韓国取寄せ'}],
    yahooA: {'TEST-NO1':0}
  });
  assert.strictEqual(result.ledgerRows.filter(r=>r.状態==='取り置き中').length, 0, '未着からは確保しない');
  assert.ok(result.issues.some(i=>i.type==='現在取寄せ未引当'&&i.sku==='TEST-NO1'));
  assert.ok(result.issues.some(i=>i.type==='未着の余り'&&i.sku==='TEST-NO1'&&i.qty===2));
});
...
test('三段階: 現物6+未着8は先行8で全数確保し供給消費は8だけ(10117608)', () => {
  const result = context.全件再計算_再構築_({
    supplies: [{ems:'EMS-NEW',code:'MRBLUE41',qty:8,arrival:'2026-07-23',row:2,status:'先行'}],
    shipped: [],
...
test('三段階: 到着済を先にFIFOし残需要だけ未着へ先行FIFOする', () => {
  const result = context.全件再計算_再構築_({
    supplies: [
      ...
      {ems:'EMS-FUT',code:'TEST-F1',qty:2,arrival:'2026-07-23',row:3,status:'先行'}
    ],
    shipped: [],
...
test('三段階: 箱到着で先行は同じ取置ID・数量のまま到着済へ昇格する', () => {
  ...
  const before = context.全件再計算_再構築_(Object.assign({}, base, {
    supplies: [{ems:'EMS-U',code:'TEST-U1',qty:1,arrival:'2026-07-23',row:2,status:'先行'}]
  }));
...
test('三段階: 先行→現物変換で解放された未着供給は別の残需要へ先行できる', () => {
  const result = context.全件再計算_再構築_({
    supplies: [{ems:'EMS-REL',code:'TEST-R1',qty:1,arrival:'2026-07-23',row:2,status:'先行'}],
    shipped: [],
    currentOrders: [

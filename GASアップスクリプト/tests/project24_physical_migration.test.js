...
['Project_24/引当.js','Project_24/取り置き計算.js','Project_24/取り置き台帳.js','Project_24/現物確認移行.js','Project_24/シート整理.js']
...
test('シート整理: 各種類の最新2世代と基準シートを残し古い監査シートだけ削除対象にする', () => {
  const names=[
    '取り置き台帳_全件再計算前_20260721_123350', // 移行の基準=保護
    '取り置き台帳_全件再計算前_20260721_132220',
    '取り置き台帳_全件再計算前_20260721_174506',
    '取り置き台帳_全件再計算前_20260722_185244',
    'EMS在庫移動台帳_全件再計算前_20260721_12335', // 99文字制限で時刻が切れた形
    'EMS在庫移動台帳_全件再計算前_20260721_13222',
    'EMS在庫移動台帳_全件再計算前_20260722_18524',
    '取り置き台帳_移行前_20260722_183000',
    '受注明細','取り置き登録','全件再計算_サマリ'
  ];
  const out=context.シート整理_削除対象_(names,2,['取り置き台帳_全件再計算前_20260721_123350']);
  assert.deepStrictEqual(JSON.parse(JSON.stringify(out)),[
    'EMS在庫移動台帳_全件再計算前_20260721_12335',
    '取り置き台帳_全件再計算前_20260721_132220'
  ]);
  assert.ok(out.indexOf('取り置き台帳_全件再計算前_20260721_123350')<0,'基準シートは保護');
  assert.ok(out.indexOf('取り置き台帳_移行前_20260722_183000')<0,'1枚しか無い種類は残す');
  assert.ok(out.indexOf('受注明細')<0,'運用シートは対象外');
});

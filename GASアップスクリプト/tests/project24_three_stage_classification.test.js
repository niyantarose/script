...
test('状態の理由: 段階数量から1列で読める文言を作る', () => {
  assert.strictEqual(context.行状態理由_(line({qty:3,現物確認済み数量:1,先行引当数量:2,先行到着予定:'2026-07-23'})),
    '注文3個/現物1個確認済み・先行2個(2026-07-23到着予定)');
  assert.strictEqual(context.行状態理由_(line({qty:3,先行引当数量:2})), '注文3個/先行2個・1個不足');
  assert.strictEqual(context.行状態理由_(line({qty:1,到着済引当数量:1})), '注文1個/到着済1個');
  assert.strictEqual(context.行状態理由_(line({})), '未引当');
  assert.strictEqual(context.行状態理由_(Object.assign(line({}),{キャンセル:true})), 'キャンセル');
  assert.strictEqual(context.行状態理由_(Object.assign(line({}),{kbn:'即納'})), '即納(店頭現物)');
});

test('行へ反映: 先行行のEMS到着予定日を行の先行到着予定へ写す', () => {
  const l={ban:'701',code:'BBB',sku:'BBBb',qty:1,kbn:'取り寄せ',届:'',キャンセル:false,別ルート済数量:0};
  const key=context.取り置き_行キー_(l);
  const plannedRow={取置ID:'R1',状態:'取り置き中',受注番号:'701',商品コード:'BBB',SKU:'BBBb',取り置き数量:1,
    取置元種別:'EMS',引当段階:'先行',元EMS番号:'EMS-F',EMS到着予定日:'2026-07-24'};
  const stageByKey={}; stageByKey[key]={現物確認済み数量:0,到着済引当数量:0,先行引当数量:1,合計確保数量:1,要移行数量:0,行内訳:[]};
  const activeByKey={}; activeByKey[key]=1;
  const activeRowsByKey={}; activeRowsByKey[key]=[plannedRow];
  const summary={activeByKey,activeRowsByKey,stageByKey};
  context.引当計画_行へ反映_([l],summary,summary,[]);
  assert.strictEqual(l.先行到着予定,'2026-07-24');
  assert.strictEqual(context.行状態理由_(l),'注文1個/先行1個(2026-07-24到着予定)');
});

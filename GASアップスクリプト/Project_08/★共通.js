 ...
 * Works照合と同名疑い警告
 * ============================================================ */
function Works照合と警告(worksKey, 作者, 作品データ, 作者別名マップ = {}) {
  ...
  if (worksKey.startsWith('原題||') && 作者) {
    ...
    const 既存作者 = 作者を名寄せ_(作品データ.keyToData[worksKey]?.作者 || '', 作者別名マップ);

    if (既存作者 && 作者比較用正規化_(現在作者) !== 作者比較用正規化_(既存作者)) {
      result.警告 = '【同名疑い・要確認】';
    }
  }
...
if (shName === '台湾雑誌') { 台湾雑誌_onEdit(e); return; }  // ← これを追加
...
function onEdit処理を実行(e, sh, cfg, 列マップ, 開始行, 行数) {
  ...
  const 言語マップ = 言語マップを取得(cfg);
  const カテゴリマップ = カテゴリマップを取得(cfg);
  const 形態マップ = 形態マップを取得(cfg);
  ...
  for (let i = 0; i < 行数; i++) {
    ...
    // タイトル未入力 or 作者未入力でも、作者だけは正規名を書き戻せるようにする
    if (!日本語タイトル || !作者) {
    ...
    const 照合結果 = Works照合と警告(worksKey, 作者, 作品データ, 作者別名マップ);
    ...
    if (照合結果.作品ID) {
      ...
      if (既存) {
        if (既存.作者) 作者 = 作者を名寄せ_(既存.作者, 作者別名マップ); // Works優先
      ...
      if (!worksKey.startsWith('原題||')) {
        ...
        if (既存worksKey && 作品データ.keyToId[既存worksKey]) {
          ...
          const 既存 = 作品データ.keyToData[worksKey];
          if (既存 && 既存.作者) 作者 = 作者を名寄せ_(既存.作者, 作者別名マップ);
          if (既存 && !原題 && 既存.原題) 原題 = 既存.原題;
        }
      }
      ...
      if (!作品ID) {
        ...
        if (!worksKey.startsWith('原題||')) {
          const titleKey = キー用正規化_(日本語タイトル);
          if (!作品データ.titleToKey[titleKey]) {
            作品データ.titleToKey[titleKey] = worksKey;
          }
        }
      }
    ...
    // 列名は cfg 側に合わせる（台湾まんがなら「サイト商品コード」に対応可能）
    const サイト商品コード列名 = cn.博客來商品コード;
    const サイト商品コード =
      サイト商品コード列名 && 列マップ[サイト商品コード列名]
        ? String(取得生(サイト商品コード列名) || '').trim()
        : '';
    ...
    if (ISBN) {
      ...
      if (ISBNトラッカー['BC:' + サイト商品コード]) 巻数警告 = '【重複注意】';
      ISBNトラッカー['BC:' + サイト商品コード] = true;
    ...
    出力SKU自動.push([SKU]);
    出力ステータス.push([ステータス]);
    出力作者.push([作者]);   // ← 追加：正規名を書き戻す
  }

  if (列マップ[cn.商品コード]) {
    sh.getRange(開始行, 列マップ[cn.商品コード], 行数, 1).setValues(出力SKU);
  }
  if (列マップ[cn.タイトル]) {
    sh.getRange(開始行, 列マップ[cn.タイトル], 行数, 1).setValues(出力タイトル);
  }
  if (列マップ[cn.作品ID]) {
    sh.getRange(開始行, 列マップ[cn.作品ID], 行数, 1).setValues(出力作品ID);
  }
  if (列マップ[cn.SKU自動]) {
    sh.getRange(開始行, 列マップ[cn.SKU自動], 行数, 1).setValues(出力SKU自動);
  }
  if (列マップ[cn.コードステータス]) {
    sh.getRange(開始行, 列マップ[cn.コードステータス], 行数, 1).setValues(出力ステータス);
  }
  if (列マップ[cn.作者]) {
    sh.getRange(開始行, 列マップ[cn.作者], 行数, 1).setValues(出力作者); // ← 追加
  }
...
function WorksKey再正規化を実行(作品シート, cfg) {
  ...
  for (let i = 0; i < データ.length; i++) {
    const t = 正規化(データ[i][2] || ''), a = 作者を名寄せ_(データ[i][3] || '', 作者別名マップ);
    if (!t || !a) continue;
  ...
  for (let i = 0; i < データ.length; i++) {
    const r = データ[i];
    const t = 正規化(r[2] || ''), a = 作者を名寄せ_(r[3] || '', 作者別名マップ), o = 正規化(r[4] || '');
    if (!t && !a) continue;
...
const 作者別名マップ = 作者別名マップを取得_(cfg);
  if (Object.keys(言語マップ).length === 0 || Object.keys(カテゴリマップ).length === 0) {
    ui.alert('言語/カテゴリマスターが見つかりません。マスターファイルIDを確認してください。'); return;
  }
  ...
  try {
    ...
    // ② 書き込み列インデックスを事前取得
    const col作品ID     = 列マップ[cn.作品ID];
const colSKU自動    = 列マップ[cn.SKU自動];
const col商品コード  = 列マップ[cn.商品コード];
const colタイトル   = 列マップ[cn.タイトル];
const colステータス  = 列マップ[cn.コードステータス];
const col作者       = 列マップ[cn.作者];
...
if (col作者)       更新データ[i][col作者 - 1]       = 作者;
    ...
    // チェックが入った行だけ列ごとに書き込む
    for (let i = 0; i < 全データ.length; i++) {
      if (全データ[i][(列マップ[cn.発行チェック] || 1) - 1] !== true) continue;
      const r = i + 2;
...
if (列マップ[cn.登録状況]) sh.getRange(r, 列マップ[cn.登録状況]).setValue('登録済み');
    }
作品データを更新_確定(作品シート, 作品データ, cfg);

    // チェックを外す
    const チェック列 = 列マップ[cn.発行チェック];
    if (チェック列) {
      const 解除 = Array(最終行 - 1).fill([false]);
      sh.getRange(2, チェック列, 最終行 - 1, 1).setValues(解除);
    }
...
const 作者別名マップ = 作者別名マップを取得_(cfg);
  ...
  try {
    ...
    const colステータス  = 列マップ[cn.コードステータス];
    const col作者       = 列マップ[cn.作者];
    const col原題       = 列マップ[cn.原題];

    const ISBNトラッカー = {}, 商品行キートラッカー = {};
    const 削除予定行 = [];

    // 変更内容を列ごとに記録 {シート行番号 → 値}
    const 変更map作品ID    = {};
    const 変更mapSKU自動   = {};
    const 変更map商品コード = {};
    const 変更mapタイトル  = {};
...
let 作者 = 作者を名寄せ_(取得(cn.作者), 作者別名マップ), 原題 = 取得(cn.原題);
const 形態 = 取得(cn.形態);
  ...
  if (ISBN確定) ISBNトラッカー[ISBN確定] = true;
  else if (博客來確定) ISBNトラッカー['BC:' + 博客來確定] = true;

  if (col作者) 変更map作者[sheetRow] = 作者;
  if (col原題 && 原題) 変更map原題[sheetRow] = 原題;

  continue; // 確定行は書き込みしない
}
      ...
      let 重複 = false;
      if (ISBN) {
        if (ISBNトラッカー[ISBN]) 重複 = true;
        else ISBNトラッカー[ISBN] = true;
      } else if (博客來コード) {
        if (ISBNトラッカー['BC:' + 博客來コード]) 重複 = true;
        else ISBNトラッカー['BC:' + 博客來コード] = true;
      } else if (原題) {
        const 商品行キー = 商品行キーを作る(原題, 取得生(cn.単巻数), 取得生(cn.セット開始), 取得生(cn.セット終了), 形態, 言語);
  ...
  if (!原題 && 既存.原題) 原題 = 正規化(既存.原題);
      ...
      const タイトル = タイトルを段階生成(cfg, 言語, カテゴリ, 形態, 日本語タイトル, 取得生(cn.単巻数), 取得生(cn.セット開始), 取得生(cn.セット終了), 作者, 原題, 取得(cn.特典メモ), 形態マップ);
      const newステータス = (日本語タイトル && 作者 && 言語 && カテゴリ) ? '商品コード(予約)' : '入力中...';
    ...
    列をまとめて書き込む(col原題,      変更map原題);

    作品データを更新_確定(作品シート, 作品データ, cfg);

    if (削除予定行.length > 0) {
      if (ui.alert('重複削除', `重複行が${削除予定行.length}件あります。削除しますか？`, ui.ButtonSet.YES_NO) === ui.Button.YES) {
        行を一括削除(sh, 削除予定行);
      }
    }
...
/* ============================================================
 * マスターデータ読み込み（外部ファイル対応）
 * cfg.マスターファイルID があれば外部ファイルのシートを参照
 * ============================================================ */
function 言語マップを取得(cfg) {
  const map = {};
  const masterSS = マスターSSを取得_(cfg);
  const sh = masterSS.getSheetByName(cfg.言語マスター名);
...
function カテゴリマップを取得(cfg) {
  const map = {};
  const masterSS = マスターSSを取得_(cfg);
  const sh = masterSS.getSheetByName(cfg.カテゴリマスター名);
  if (!sh || sh.getLastRow() < 2) return map;
  const data = sh.getRange(2, 1, sh.getLastRow() - 1, 2).getValues();
  for (const [n, c] of data) { const name = String(n || '').trim(), code = String(c || '').trim(); if (name && code) map[name] = code; }
  return map;
...
function 形態マップを取得(cfg) {
  ...
  const masterSS = マスターSSを取得_(cfg);
  const sh = masterSS.getSheetByName(cfg.形態マスター名 || '形態マスターシート');
...
function プルダウン更新を実行(cfg) {
  ...
  const カテゴリデータ = カテゴリデータを色付きで取得(cfg);
  ...
  // 配送パターン（マスターファイルの配送パターンマスターシートから取得）
  if (cn.配送パターン && 列マップ[cn.配送パターン]) {
    try {
      const 配送マスターSh = マスターSSを取得_(cfg).getSheetByName('配送パターンマスター');
      if (配送マスターSh && 配送マスターSh.getLastRow() >= 2) {
        const 配送値 = 配送マスターSh.getRange(2, 1, 配送マスターSh.getLastRow() - 1, 1)
          .getValues().flat().map(v => String(v).trim()).filter(v => v);
        if (配送値.length > 0) {
          sh.getRange(2, 列マップ[cn.配送パターン], 最終行 - 1, 1).setDataValidation(
            SpreadsheetApp.newDataValidation().requireValueInList(配送値, true).build()
          );
        }
      }
    } catch(err) { Logger.log('配送マスター取得エラー: ' + err.message); }
  }
...
function 言語マップをSSから取得_(masterSS, cfg) {
  const map = {};
  const sh = masterSS.getSheetByName(cfg.言語マスター名);
  if (!sh || sh.getLastRow() < 2) return map;
  sh.getRange(2, 1, sh.getLastRow() - 1, 2).getValues()
    .forEach(([n, c]) => { const name = String(n||'').trim(), code = String(c||'').trim(); if (name && code) map[name] = code; });
  return map;
}

function カテゴリマップをSSから取得_(masterSS, cfg) {
  const map = {};
  const sh = masterSS.getSheetByName(cfg.カテゴリマスター名);
  if (!sh || sh.getLastRow() < 2) return map;
  sh.getRange(2, 1, sh.getLastRow() - 1, 2).getValues()
...
function 形態マップをSSから取得_(masterSS, cfg) {
  ...
  const sh = masterSS.getSheetByName(cfg.形態マスター名 || '形態マスターシート');
  if (!sh || sh.getLastRow() < 2) return result;
  sh.getRange(2, 1, sh.getLastRow() - 1, 5).getValues().forEach(row => {
    const 名前 = String(row[0] || '').trim();
    if (!名前) return;
    result.コード[名前]       = String(row[1] || '').trim();
    result.特典テキスト[名前] = String(row[2] || '').trim();

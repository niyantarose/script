...
// （シート側の「採番を巻き戻す」にWebアプリも追従できる）
scriptProps['台湾書籍系_作品ID_ハイウォーター'] = '170';
sheetExists = true;
sharedCellValue = '167';
eq('巻き戻し: セル(167)がprops(170)より優先', ctx.bookCodeNextUnusedWorkId_(fakeSS, makeRuntime(['0005', '0165'])), '0168');

// ケース6: セルを走査max未満まで下げても、シート上の使用中IDは絶対に越えない
scriptProps['台湾書籍系_作品ID_ハイウォーター'] = '0';
sharedCellValue = '100';
eq('巻き戻し: 走査max(165)が下限', ctx.bookCodeNextUnusedWorkId_(fakeSS, makeRuntime(['0005', '0165'])), '0166');

// ケース7: 解放プールがあれば小さい順に優先消費（HWは動かさない）
scriptProps['台湾書籍系_作品ID_ハイウォーター'] = '168';
sharedCellValue = '168';
poolCellValue = '0167,0166';
eq('プール: 最小の0166を消費', ctx.bookCodeNextUnusedWorkId_(fakeSS, makeRuntime(['0005', '0165'])), '0166');
...
eq('プール: 使用済み0166を飛ばして0167', ctx.bookCodeNextUnusedWorkId_(fakeSS, makeRuntime(['0005', '0166'])), '0167');
...
sharedCellValue = '165';
poolCellValue = '';
eq('プール空: 従来のmax+1', ctx.bookCodeNextUnusedWorkId_(fakeSS, makeRuntime(['0005', '0165'])), '0166');

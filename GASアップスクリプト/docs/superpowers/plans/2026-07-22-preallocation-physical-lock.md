...
> 進捗メモ(2026-07-22 Claude, eaf3905): 物理オペ対象行_/便締め先行残検査_を取り置き計算.jsへ実装し
> ⑤(先行残で停止・未ピックは物理行のみ・台帳読込失敗は停止)とYahoo出力(未着余り除外・未知EMS中止)を
> ガード済み。テスト=physical_operation_guard+yahoo_stock_export。
> **残り**: 受注明細個別ボタンの個別引当が未着供給を候補にしない除外(Step 5後半)と、
> 納品書・ピック系出力への物理出荷可の明示(Step 3後半)。

- [x] **Step 1: 物理境界のREDテストを書く**
...
> リリース検証(2026-07-22 Claude): Step 1=15スイート全PASS / Step 2=全Project_24ファイル構文エラー0件
> / Step 3=差分はProject_24・tests・docsのみ(20ファイル +2515/-109)、git diff --check クリーン。
> Step 4のsafe_pushはユーザーのタイミング確認待ち(表示モード既定変更・全件反映ガード発動・
> 同期基準の古さによる衝突可能性があるため、作業の切れ目で実行するのが安全)。

- [x] **Step 1: 全Project_24テストを実行**

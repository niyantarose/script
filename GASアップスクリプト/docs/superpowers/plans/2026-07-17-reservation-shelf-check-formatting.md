...
**Goal:** 「取り置き登録」で商品名由来の予約除外を廃止し、注文単位の太線と棚確認ステータス別の即時行色を追加する。
...
**Step 1: 旧予約テストを新仕様の回帰テストへ置き換える**

`取り置き_予約判定_`を呼ぶ旧テストを削除し、商品名・選択肢や入力オブジェクトの予約フラグに関係なく、初期候補の `棚確認` が空欄になるテストへ置き換える。
...
test('商品名や選択肢の予約表記では棚確認を自動予約にしない', () => {
  ...
  assert.deepStrictEqual(rows.map(r=>r.棚確認),['','']);
});
```
...
Expected: `予約受付終了` を含む行が誤って除外されるテストだけが失敗する。

**Step 5: 最小実装で完全一致ルールへ直す**
...
Claude Codeの途中差分に含まれる次の変更はそのまま採用する。
...
test('要棚確認を優先しても同じ注文の商品行を分断しない', () => {
  ...
  assert.deepStrictEqual(rows.map(r=>r.受注番号),['100','100','200','300']);
});
```
...
assert.deepStrictEqual(
  Array.from(context.取り置き_注文境界A1_([
...
assert.deepStrictEqual(defs.map(d=>[d.値,d.背景]),[
  ['発送待ち','#cfe2f3'],
  ['部分在庫','#d9ead3'],
  ['出荷済み','#f4cccc'],
  ['未着','#d9d9d9'],

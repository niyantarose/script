...
- `予約中` は除外
- `■出荷GO` は除外
- `旧入荷日 + 予約中`、`部分包装 + 予約中`、`部分包装 + 出荷GO` も受注ステータスを優先して除外
- `出荷待/取寄せ` は旧入荷日が空でも表示
...
過去日、日付不明、予約表記なしはfalseにする。候補収集時に商品名・選択肢を判定して予約フラグへ渡し、初期候補で自動 `予約` を設定する。ただし旧入荷日または `部分包装` がある行は空欄にする。この例外は商品名・選択肢由来の自動予約だけに適用し、後段では `予約中` / `出荷GO` を常に除外する。`予約` を判断済み記憶の対象へ含める変更は維持する。
...
- `旧入荷日 + 予約中`、`部分包装 + 予約中`、`部分包装 + 出荷GO` が無条件除外される組み合わせテストを追加する。
- 候補作成前に、取り置き登録の管理対象データ領域について背景色・格子・太線を一括クリアし、現在候補へだけ再設定する。
- 候補縮小時と0件時に旧背景・旧罫線が残らないテストを追加する。

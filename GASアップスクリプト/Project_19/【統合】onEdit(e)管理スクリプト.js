function onEdit(e) {
  ...
  try {
    ...
    if (sheetName === HATCHU_CFG.SHEET_NAME) {
      autofillHachu(e);   // ★この1行を追加(コード/業者を入れたらマスタから値を入れる)
商品コードと業者と商品名と価格がそろったら自動でマスタ登録(e);  // ★追加(行がそろったら自動でマスタ登録)
  ...
  if (!onlyCheckboxCol) {
    ...
    const startRow = Math.max(HATCHU_CFG.START_ROW, range.getRow() - 2);
    const endRow = Math.min(
      sh.getMaxRows(),
      range.getRow() + range.getNumRows() + 2
    );
  ...
  発注_drawGroupBorders_(sh);   // ★この1行を追加(編集のたびに太枠も付け直す)
...
function 商品コードと業者と商品名と価格がそろったら自動でマスタ登録(e){
  ...
  const watch = [CFG.HACHU_CODE, CFG.HACHU_VENDOR, CFG.HACHU_NAME, CFG.HACHU_ITEM, CFG.HACHU_PRICE];
  if (!watch.some(w => sCol <= w && w <= eCol)) return;   // 関係ない列はスルー

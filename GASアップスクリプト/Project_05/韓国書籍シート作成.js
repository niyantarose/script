/**
 * 【旧版・使用禁止】
 * 旧スキーマ（発番発行列なし・管理コード/作品番号など旧体系）のシート作成。
 * 新版は 韓国書籍.js の 韓国書籍シートを作成() に移行済み。
 * 同名関数の二重定義だとファイル順次第で旧版がメニューの
 * 「韓国書籍シートを作成」を乗っ取り、フレームワーク非対応の
 * シートが作られてしまうため、関数名を変更して退避した。
 * 不要と確認できたらこのファイルごと削除してよい。
 */
function 韓国書籍シートを作成_旧版_使用禁止_() {

...
// 【Bワークフロー】行の日本語タイトル(G)を手編集したとき、その値をWorks(書籍専用)へ書き戻す。
// 作品ID(W)（自動）でWorks行を特定して日本語タイトルを上書き。以降の1行補完(Works最優先)が
// 更新後のWorksを読むため、編集が元に戻らず、同一作品の全巻へ伝播する。
function 台湾書籍系_行の日本語タイトルをWorksへ書き戻す_(sh, row, 設定) {
  if (!sh || !設定 || !設定.作品シート名) return;
  const scol = 台湾書籍系_列マップを取得_(sh);
  const jpCol = scol[台湾書籍系_正規化文字列_('日本語タイトル')];
  const widCol = scol[台湾書籍系_正規化文字列_('作品ID(W)（自動）')];
  if (!jpCol || !widCol) return;
  const 新jp = String(sh.getRange(row, jpCol).getValue() || '').trim();
  if (!新jp) return; // 空にした場合は書き戻さない
  const wid = 台湾書籍系_作品ID4桁を取得_(sh.getRange(row, widCol).getValue());
  if (!wid) return;  // 作品ID未確定の行は対象外（1行補完側の取得/作成に任せる）
  const wsh = SpreadsheetApp.getActive().getSheetByName(設定.作品シート名);
  if (!wsh || wsh.getLastRow() < 2) return;
  const wheaders = wsh.getRange(1, 1, 1, wsh.getLastColumn()).getValues()[0]
    .map(v => 台湾書籍系_正規化文字列_(v));
  const wc = {};
  wheaders.forEach((h, i) => { if (h) wc[h] = i; });
  const idKey = 台湾書籍系_正規化文字列_('作品ID');
  const jpKey = 台湾書籍系_正規化文字列_('日本語タイトル');
  if (wc[idKey] == null || wc[jpKey] == null) return;
  const data = wsh.getRange(2, 1, wsh.getLastRow() - 1, wsh.getLastColumn()).getValues();
  for (let i = 0; i < data.length; i++) {
    if (台湾書籍系_作品ID4桁を取得_(data[i][wc[idKey]]) === wid) {
      const cur = String(data[i][wc[jpKey]] || '').trim();
      if (cur !== 新jp) wsh.getRange(i + 2, wc[jpKey] + 1).setValue(新jp);
      return;
    }
  }
}
...
function 台湾書籍系_1行補完_共通_(sh, row, 設定, options = {}) {
  ...
  if (works) {
    ...
    // 【日本語タイトルの優先順位】Works（最優先）＞ ページ直下の日本語題 ＞ 各サイト検索。
    // ページ/サイトの順は拡張側で確定済み。ここでは Works に実値があれば、
    // 現在値（ページ/サイト由来や「登録なし」等のステータス値）より優先して Works 値で上書きする。
    // → 同一作品の全巻で日本語タイトルが Works に統一される。
    if (台湾書籍系_正規化文字列_(works.日本語タイトル)) {
...
function 台湾書籍系_onEdit_共通_(e, 設定) {
  ...
  try {
    // 【Bワークフロー】日本語タイトル(G)を手編集した行は、まず新値をWorksへ書き戻してから補完する。
    // 後段の1行補完(Works最優先上書き)が更新後のWorksを読むため、編集が元に戻らず全巻へ伝播する。
    const 日本語タイトル列 = 列[台湾書籍系_正規化文字列_('日本語タイトル')];
    if (日本語タイトル列 && 編集開始列 <= 日本語タイトル列 && 日本語タイトル列 <= 編集終了列) {
      for (let wr = 開始行; wr < 開始行 + 行数; wr++) {
        if (wr < 2) continue;
        台湾書籍系_行の日本語タイトルをWorksへ書き戻す_(sh, wr, 設定);
      }
    }

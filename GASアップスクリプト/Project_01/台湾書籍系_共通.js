...
/* ============================================================
 * 確定発行バッチ文脈（高速化）
 * 確定発行の一括処理の「間だけ」有効化する読取キャッシュ。
 * ・sheetValues : 商品シート 2..lastRow の 2D 配列。各行処理後にその1行だけ差し替えて追従する。
 * ・works/worksCol : Worksシートの 2..lastRow 2D 配列と列マップ。新規Works作成時に push して追従する。
 * 文脈が無いとき（onEdit 等の通常経路）は各関数が従来どおり毎回フル読みするため挙動は不変。
 * ロールバックは _TWBK_確定Ctx設定_ の呼び出し1行を外すだけで従来動作へ戻る（加算的変更）。
 * ============================================================ */
let _TWBK_確定Ctx_ = null; // { sheetName, lastCol, sheetValues, worksSheetName, worksCol, works }
function _TWBK_確定Ctx設定_(ctx) { _TWBK_確定Ctx_ = ctx || null; }
...
function 台湾書籍系_Worksから取得_(設定, 原題タイトル, 日本語タイトル = '', 作者 = '', 媒体コード = '') {
  ...
  let data, col;
  // バッチ文脈があれば Worksシートの全読みをキャッシュ配列で置き換える（検索条件・戻り値の形は不変）。
  // 文脈が無ければ従来どおりシートを読む。
  if (_TWBK_確定Ctx_ && _TWBK_確定Ctx_.worksSheetName === 設定.作品シート名 && _TWBK_確定Ctx_.works && _TWBK_確定Ctx_.worksCol) {
    data = _TWBK_確定Ctx_.works;
    col = _TWBK_確定Ctx_.worksCol;
  } else {
    const ss = SpreadsheetApp.getActive();
    ...
    data = sh.getRange(2, 1, sh.getLastRow() - 1, sh.getLastColumn()).getValues();
  }
// 1) 完全一致の直前に
...
function 台湾書籍系_同一作品行から情報を取得_(sh, currentRow, 列, 値, 実列名) {
  ...
  if (!sh || !targetKeys.length) return null;
  const targetMedia = 台湾書籍系_媒体コードを生成_(値, sh.getName());

  let values;
  // バッチ文脈があれば商品シートの全読みをキャッシュ配列で置き換える（走査ロジックは不変）。
  // 文脈が無ければ従来どおりシートを読む。
  if (_TWBK_確定Ctx_ && _TWBK_確定Ctx_.sheetName === sh.getName() && _TWBK_確定Ctx_.sheetValues) {
    values = _TWBK_確定Ctx_.sheetValues;
  } else {
...
function 台湾書籍系_ローカル確定発行_(シート名, 設定) {
  ...
  try {
    ...
    // 【高速化】確定発行の間だけバッチ文脈を有効化する。
    //   従来は対象N行ごとに「商品シート全読み＋Works全読み」が最大2N回走っていた。
    //   これを『初回に商品シート1回＋Works1回の全読み』＋『各行処理後にその1行だけ再読込』に置換する。
    //   文脈が無い経路（onEdit 等）は従来どおりで挙動不変。finally で必ずクリアする。
    // ※ 文脈構築を try 内先頭に置くことで、途中失敗しても finally で確実にクリアされる（module-level漏洩防止）。
    const _商品Vals = sh.getRange(2, 1, numRows, lastCol).getValues();
    let _worksVals = [];
    const _worksCol = {};
    try {
      const _wsh = ss.getSheetByName(設定.作品シート名);
      if (_wsh && _wsh.getLastRow() >= 2) {
        const _wHeaders = _wsh.getRange(1, 1, 1, _wsh.getLastColumn()).getValues()[0]
          .map(v => 台湾書籍系_正規化文字列_(v));
        _wHeaders.forEach((h, i) => { if (h) _worksCol[h] = i; });
        _worksVals = _wsh.getRange(2, 1, _wsh.getLastRow() - 1, _wsh.getLastColumn()).getValues();
      }
    } catch (e) {
      Logger.log('確定発行バッチ文脈: Works読込失敗（従来経路に縮退） ' + e);
    }
    _TWBK_確定Ctx設定_({
      sheetName: シート名,
      lastCol: lastCol,
      sheetValues: _商品Vals,
      worksSheetName: 設定.作品シート名,
      worksCol: _worksCol,
      works: _worksVals,
    });
    ...
    対象行.forEach(row => {
      ...
      // その行の最新値（採番/コード生成の結果）を文脈へ差し替える。
      // これで同一作品の後続行が、最新の作品IDをシート再読込なしで参照できる（ID一貫性の要）。
      _商品Vals[row - 2] = sh.getRange(row, 1, 1, lastCol).getValues()[0];
    });
    ...
    _TWBK_確定Ctxクリア_(); // バッチ文脈は必ずクリア（onEdit 等の後続実行へ漏らさない）
    lock.releaseLock();
  }
}
...
if (!作品名) {
  ...
  sh.getRange(writeRow, 1, 1, 設定.作品列数).setValues([新Works行]);

  // バッチ文脈があれば、新規Works行を文脈のworks配列にも push して追従させる。
  // これで同一バッチ内の後続の同一作品行が、シート再読込なしで同じ作品IDを引ける（ID一貫性の要）。
  if (_TWBK_確定Ctx_ && _TWBK_確定Ctx_.worksSheetName === 設定.作品シート名 && _TWBK_確定Ctx_.works) {
    _TWBK_確定Ctx_.works.push(新Works行.slice());
  }
...
function 台湾書籍系_欠番管理シートを更新_() {
  ...
  try {
    ...
    try {
      ...
      ボタンエラー = String(e && e.message ? e.message : e);
    }

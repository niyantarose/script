 ...
 * 仕入れ計算機 v2（中国・韓国・台湾）
 *
 * ・原価帯ごとの目標粗利率は黄色で設定（あなたが決める）
 * ・原価が出たら、その帯の利益率を「適用目標粗利率」に自動適用
 * ・実際のうわのせも黄色で手入力（初期値は推奨）
 * ・韓国: 重さ(g)を直接入力して原価・おすすめ売値を自動計算
 */
...
/**
 * 【別設定】原価帯別の目標粗利率
 * 日常の黄色入力（価格・うわのせ等）とは別欄。ここを変えると適用率が自動で変わる。
 */
...
function 計算機_きれいなシートを作成() {
  ...
  // 台湾: 目標粗利率 B10(=適用連動) / うわのせ B11 / 帯 B14-B18 / 手数料 B21 / 原価 B23 / 適用 B24
  const 台湾 = [
    ['台湾 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['目標粗利率', '=IFERROR(B24,' + twGoal + ')', 'input', '0%', '通常は原価帯自動に連動。今回だけ変えたい時は数値で上書き'],
    ['実際のうわのせ', twUwa, 'input', '¥#,##0', '手で調整。初期値は推奨(10円丸め)'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_()).concat([
    ['', '', 'blank'],
    ['── 自動計算 ──', '', 'section'],
    ['手数料合計', '=B8+B9', 'auto', '0%', 'モール+消費税'],
    ['送料(元)', '=B4*B7', 'auto', '0.0', '本の価格×送料率'],
    ['原価(円)', '=ROUND((B4+B22)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B23', 'B14', 'B15', 'B16', 'B17', 'B18'), 'auto', '0%', '【別設定】の帯×原価から自動'],
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B23/(1-B10-B21),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B27,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B27,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B28-B23', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B29-B23', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B28-B28*B21-B23', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B29-B29*B21-B23', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B23+B11', 'actual', '¥#,##0', '式: 原価(B23)+うわのせ(B11)'],
    ['実質粗利額', '=B36-B36*B21-B23', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B37)/B36', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B38<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ]);

  // 中国: 目標粗利率 B11 / うわのせ B12 / 帯 B15-B19 / 手数料 B22 / 原価 B24 / 適用 B25
  const 中国 = [
    ['中国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['消費税', 0.1, 'input', '0%', '0.1=10%'],
    ['目標粗利率', '=IFERROR(B25,' + cnGoal + ')', 'input', '0%', '通常は原価帯自動に連動。今回だけ変えたい時は数値で上書き'],
    ['実際のうわのせ', cnUwa, 'input', '¥#,##0', '手で調整。初期値は推奨(10円丸め)'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_()).concat([
    ['', '', 'blank'],
    ['── 自動計算 ──', '', 'section'],
    ...
    ['原価(円)', '=ROUND((B4+B23)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B24', 'B15', 'B16', 'B17', 'B18', 'B19'), 'auto', '0%', '【別設定】の帯×原価から自動'],
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B24/(1-B11-B22),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B28,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B28,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B29-B24', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B30-B24', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B29-B29*B22-B24', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B30-B30*B22-B24', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B24+B12', 'actual', '¥#,##0', '式: 原価(B24)+うわのせ(B12)'],
    ['実質粗利額', '=B37-B37*B22-B24', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B38)/B37', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B39<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ]);

  // 韓国: 目標粗利率 B11 / うわのせ B12 / 帯 B15-B19 / 手数料 B22 / 原価 B23 / 適用 B24
  const 韓国 = [
    ['韓国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['モール手数料', 0.09, 'input', '0%', 'Yahoo等'],
    ['消費税', 0.1, 'input', '0%', '0.1=10%'],
    ['目標粗利率', '=IFERROR(B24,' + krGoal + ')', 'input', '0%', '通常は原価帯自動に連動。今回だけ変えたい時は数値で上書き'],
    ['実際のうわのせ', krUwa, 'input', '¥#,##0', '手で調整。初期値は推奨(10円丸め)'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_()).concat([
    ['', '', 'blank'],
    ['── 自動計算 ──', '', 'section'],
    ...
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B23', 'B15', 'B16', 'B17', 'B18', 'B19'), 'auto', '0%', '【別設定】の帯×原価から自動'],
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B23/(1-B11-B22),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B27,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B27,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B28-B23', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B29-B23', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B28-B28*B22-B23', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B29-B29*B22-B23', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B23+B12', 'actual', '¥#,##0', '式: 原価(B23)+うわのせ(B12)'],
    ['実質粗利額', '=B36-B36*B22-B23', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B37)/B36', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B38<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ]);
  ...
  ss.toast('台湾・韓国・中国 計算機 v2 を再作成しました（原価帯設定→自動適用）');
}

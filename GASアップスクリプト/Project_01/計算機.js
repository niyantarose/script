...
function 計算機_きれいなシートを作成() {
  ...
  const 台湾 = [
    ...
    ['原価(円)', '=ROUND((B4+B12)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['適用目標粗利率(自動)', 計算機_原価帯粗利率式_('B13'), 'auto', '0%', '原価帯で自動決定'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯参考行_()).concat([
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B13/(1-B14-B11),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ...
    ['おすすめ時粗利額(10円丸め)', '=B26-B26*B11-B13', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B27-B27*B11-B13', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整（実際・青）──', '', 'section'],
    ['実際のうわのせ', twUwa, 'actual', '¥#,##0', '↑推奨をコピーして手で調整'],
    ['実際売値', '=B13+B34', 'actual', '¥#,##0', '式: 原価(B13)+うわのせ(B34)'],
    ['実質粗利額', '=B35-B35*B11-B13', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B36)/B35', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B37<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ]);
  ...
  const 中国 = [
    ...
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B14/(1-B15-B12),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],

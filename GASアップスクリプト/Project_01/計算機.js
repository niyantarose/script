...
function 計算機_きれいなシートを作成() {
  ...
  // 原価帯売値 B25-B26 / 自動計算: 手数料B29 送料B30 原価B31 適用B32 / 帯 B35-B39
  const 台湾 = [
    ['台湾 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['目標粗利率', '=IFERROR(B32,' + twGoal + ')', 'input', '0%', '通常は原価帯自動に連動。今回だけ変えたい時は数値で上書き'],
    ['実際のうわのせ', twUwa, 'input', '¥#,##0', '手で調整。初期値はおすすめ(10円丸め)基準'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B31+B11', 'actual', '¥#,##0', '式: 原価(B31)+うわのせ(B11)'],
    ['実質粗利額', '=B14-B14*B29-B31', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=IFERROR(B15/B14,0)', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B16<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
    ['', '', 'blank'],
    ['── おすすめ（緑）──', '', 'section'],
    ['おすすめ売値(10円丸め)', '=IFERROR(ROUND(B31/(1-B10-B29),-1),0)', 'result', '¥#,##0', '目標粗利率で計算 例:8480'],
    ['おすすめ売値(100円丸め)', '=IFERROR(ROUND(B31/(1-B10-B29),-2),0)', 'result', '¥#,##0', '目標粗利率で計算 例:8500'],
    ['', '', 'blank'],
  ...
  // 原価帯売値 B25-B26 / 自動: 手数料B29 送料B30 原価B31 適用B32 / 帯 B35-B39
  const 中国 = [
    ['中国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
  ...
  // 韓国
  // 入力 B4-B12 / 微調整 B15-B18 / おすすめ B21-B22
  // 原価帯売値 B25-B26 / 自動: 手数料B29 原価B30 適用B31 / 帯 B34-B38
  const 韓国 = [
    ['韓国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],

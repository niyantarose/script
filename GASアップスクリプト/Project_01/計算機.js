...
function 計算機_きれいなシートを作成() {
  ...
  // 自動価格: 手数料B24 送料B25 原価B26 適用B27 自動10=B28 自動100=B29 / 帯 B32-B36
  const 台湾 = [
    ['台湾 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['', '', 'blank'],
    ['── おすすめ（緑）──', '', 'section'],
    ['おすすめ売値(10円丸め)', '=IFERROR(ROUND(B26/(1-B10-B24),-1),0)', 'result', '¥#,##0', '例:8480'],
    ['おすすめ売値(100円丸め)', '=IFERROR(ROUND(B26/(1-B10-B24),-2),0)', 'result', '¥#,##0', '例:8500'],
    ['', '', 'blank'],
    ['── 自動価格計算結果（原価帯）──', '', 'section'],
    ['手数料合計', '=B8+B9', 'auto', '0%', 'モール+消費税'],
    ['送料(元)', '=B4*B7', 'auto', '0.0', '本の価格×送料率'],
    ['原価(円)', '=ROUND((B4+B25)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B26', 'B32', 'B33', 'B34', 'B35', 'B36'), 'auto', '0%', '一番下の帯設定×原価から自動'],
    ['原価帯売値(10円丸め)', '=IFERROR(ROUND(B26/(1-B27-B24),-1),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['原価帯売値(100円丸め)', '=IFERROR(ROUND(B26/(1-B27-B24),-2),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_());

  // 中国
  // 入力 B4-B12 / 微調整 B15-B18 / おすすめ B21-B22
  // 自動: 手数料B25 送料B26 原価B27 適用B28 自動10=B29 自動100=B30 / 帯 B33-B37
  const 中国 = [
    ['中国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['', '', 'blank'],
    ['── おすすめ（緑）──', '', 'section'],
    ['おすすめ売値(10円丸め)', '=IFERROR(ROUND(B27/(1-B11-B25),-1),0)', 'result', '¥#,##0', '例:8480'],
    ['おすすめ売値(100円丸め)', '=IFERROR(ROUND(B27/(1-B11-B25),-2),0)', 'result', '¥#,##0', '例:8500'],
    ['', '', 'blank'],
    ['── 自動価格計算結果（原価帯）──', '', 'section'],
    ['手数料合計', '=B8+B9+B10', 'auto', '0%', 'モール+カード+消費税'],
    ['送料(元)', '=B4*B7', 'auto', '0.0', '商品価格×送料率'],
    ['原価(円)', '=ROUND((B4+B26)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B27', 'B33', 'B34', 'B35', 'B36', 'B37'), 'auto', '0%', '一番下の帯設定×原価から自動'],
    ['原価帯売値(10円丸め)', '=IFERROR(ROUND(B27/(1-B28-B25),-1),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['原価帯売値(100円丸め)', '=IFERROR(ROUND(B27/(1-B28-B25),-2),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_());

  // 韓国
  // 入力 B4-B12 / 微調整 B15-B18 / おすすめ B21-B22
  // 自動: 手数料B25 原価B26 適用B27 自動10=B28 自動100=B29 / 帯 B32-B36
  const 韓国 = [
    ['韓国 仕入れ計算機 v2', '', 'title'],
    ['', '', 'blank'],
    ['── 入力（黄色・毎回ここ）──', '', 'section'],
    ...
    ['', '', 'blank'],
    ['── おすすめ（緑）──', '', 'section'],
    ['おすすめ売値(10円丸め)', '=IFERROR(ROUND(B26/(1-B11-B25),-1),0)', 'result', '¥#,##0', '例:8480'],
    ['おすすめ売値(100円丸め)', '=IFERROR(ROUND(B26/(1-B11-B25),-2),0)', 'result', '¥#,##0', '例:8500'],
    ['', '', 'blank'],
    ['── 自動価格計算結果（原価帯）──', '', 'section'],
    ['手数料合計', '=B9+B10', 'auto', '0%', 'モール+消費税'],
    ['原価(円)', '=ROUND(B4*B6+B5*B8,0)', 'auto', '¥#,##0', '価格×レート+重さ×係数'],
    ['適用目標粗利率(原価帯から自動)', 計算機_原価帯粗利率式_('B26', 'B32', 'B33', 'B34', 'B35', 'B36'), 'auto', '0%', '一番下の帯設定×原価から自動'],
    ['原価帯売値(10円丸め)', '=IFERROR(ROUND(B26/(1-B27-B25),-1),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['原価帯売値(100円丸め)', '=IFERROR(ROUND(B26/(1-B27-B25),-2),0)', 'result', '¥#,##0', '原価帯の適用率で自動計算'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯設定行_());
...
function 計算機_buildSheet_(ss, name, rows) {
  ...
  rows.forEach(function (r, idx) {
    ...
    if (type === 'result' && (label.indexOf('おすすめ') >= 0 || label.indexOf('原価帯売値') >= 0)) {

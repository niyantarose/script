...
/** 原価帯別目標粗利率（黄色＝設定。適用は原価から自動） */
function 計算機_原価帯設定行_() {
  return [
    ['── 原価帯別目標粗利率（黄色＝あなたの設定）──', '', 'section'],
    ['〜 2,000円まで', 0.35, 'input', '0%', 'この帯の目標粗利率（書き換え可）'],
    ['〜 6,000円まで', 0.25, 'input', '0%', 'この帯の目標粗利率（書き換え可）'],
    ['〜 12,000円まで', 0.2, 'input', '0%', 'この帯の目標粗利率（書き換え可）'],
    ['〜 20,000円まで', 0.1, 'input', '0%', 'この帯の目標粗利率（書き換え可）'],
    ['20,000円超', 0.08, 'input', '0%', 'この帯の目標粗利率（書き換え可）'],
  ];
}
...
function 計算機_きれいなシートを作成() {
  ...
  const 台湾 = [
    ...
    ['目標粗利率', twGoal, 'input', '0%', '手入力。初期値は原価帯の目安'],
    ['実際のうわのせ', twUwa, 'input', '¥#,##0', '手で調整。初期値は推奨(10円丸め)'],
    ['', '', 'blank'],
    ['── 自動計算 ──', '', 'section'],
    ['手数料合計', '=B8+B9', 'auto', '0%', 'モール+消費税'],
    ['送料(元)', '=B4*B7', 'auto', '0.0', '本の価格×送料率'],
    ['原価(円)', '=ROUND((B4+B15)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['原価帯参考粗利率', 計算機_原価帯粗利率式_('B16'), 'auto', '0%', '参考のみ。計算は黄色の目標粗利率'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯参考行_()).concat([
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B16/(1-B10-B14),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B28,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B28,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B29-B16', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B30-B16', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B29-B29*B14-B16', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B30-B30*B14-B16', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B16+B11', 'actual', '¥#,##0', '式: 原価(B16)+うわのせ(B11)'],
    ['実質粗利額', '=B37-B37*B14-B16', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
  ...
  const 中国 = [
    ...
    ['消費税', 0.1, 'input', '0%', '0.1=10%'],
    ['目標粗利率', cnGoal, 'input', '0%', '手入力。初期値は原価帯の目安'],
    ['実際のうわのせ', cnUwa, 'input', '¥#,##0', '手で調整。初期値は推奨(10円丸め)'],
    ['', '', 'blank'],
    ['── 自動計算 ──', '', 'section'],
    ['手数料合計', '=B8+B9+B10', 'auto', '0%', 'モール+カード+消費税'],
    ['送料(元)', '=B4*B7', 'auto', '0.0', '商品価格×送料率'],
    ['原価(円)', '=ROUND((B4+B16)*B5,0)', 'auto', '¥#,##0', '(価格+送料)×レート'],
    ['原価帯参考粗利率', 計算機_原価帯粗利率式_('B17'), 'auto', '0%', '参考のみ。計算は黄色の目標粗利率'],
    ['', '', 'blank'],
  ].concat(計算機_原価帯参考行_()).concat([
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B17/(1-B11-B15),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B29,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B29,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B30-B17', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B31-B17', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B30-B30*B15-B17', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B31-B31*B15-B17', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B17+B12', 'actual', '¥#,##0', '式: 原価(B17)+うわのせ(B12)'],
    ['実質粗利額', '=B38-B38*B15-B17', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
  ...
  const 韓国 = [
    ...
    ['モール手数料', 0.09, 'input', '0%', 'Yahoo等'],
    ['消費税', 0.1, 'input', '0%', '0.1=10%'],
    ['目標粗利率', krGoal, 'input', '0%', '手入力。初期値は原価帯の目安'],
    ...
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B16/(1-B11-B15),0)', 'result', '¥#,##0', '式: 原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B28,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B28,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B29-B16', 'result', '¥#,##0', '式: おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B30-B16', 'result', '¥#,##0', '式: おすすめ売値(100円)-原価'],
    ['おすすめ時粗利額(10円丸め)', '=B29-B29*B15-B16', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['おすすめ時粗利額(100円丸め)', '=B30-B30*B15-B16', 'result', '¥#,##0', 'この売値なら手数料後にこれだけ残る'],
    ['', '', 'blank'],
    ['── 微調整結果（青）──', '', 'section'],
    ['実際売値', '=B16+B12', 'actual', '¥#,##0', '式: 原価(B16)+うわのせ(B12)'],
    ['実質粗利額', '=B37-B37*B15-B16', 'actual', '¥#,##0', '式: 売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B38)/B37', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B39<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ]);
  ...
  ss.toast('台湾・韓国・中国 計算機 v2 を再作成しました（目標粗利率は黄色で手入力）');
}

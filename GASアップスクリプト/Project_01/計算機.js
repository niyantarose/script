...
function 計算機_きれいなシートを作成() {
  ...
  const 台湾 = [
    ...
    ['理論売値', '=ROUND(B15/(1-B5-B13),0)', 'result', '¥#,##0', '=原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B18,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B18,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B19-B15', 'result', '¥#,##0', '=おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B20-B15', 'result', '¥#,##0', '=おすすめ売値(100円)-原価'],
    ['', '', 'blank'],
    ['── 微調整（実際・青）──', '', 'section'],
    ['実際のうわのせ', twUwa, 'actual', '¥#,##0', '↑推奨をコピーして手で調整'],
    ['実際売値', '=B15+B25', 'actual', '¥#,##0', '=原価(B15)+うわのせ(B25)'],
    ['実質粗利額', '=B26-B26*B13-B15', 'actual', '¥#,##0', '=売値-売値×手数料合計-原価'],
  ...
  const 中国 = [
    ...
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B16/(1-B5-B14),0)', 'result', '¥#,##0', '=原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B19,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B19,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B20-B16', 'result', '¥#,##0', '=おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B21-B16', 'result', '¥#,##0', '=おすすめ売値(100円)-原価'],
    ['', '', 'blank'],
    ['── 微調整（実際・青）──', '', 'section'],
    ['実際のうわのせ', cnUwa, 'actual', '¥#,##0', '↑推奨をコピーして手で調整'],
    ['実際売値', '=B16+B26', 'actual', '¥#,##0', '=原価(B16)+うわのせ(B26)'],
    ['実質粗利額', '=B27-B27*B14-B16', 'actual', '¥#,##0', '=売値-売値×手数料合計-原価'],
  ...
  const 韓国 = [
    ...
    ['', '', 'blank'],
    ['── おすすめ（目標粗利率から・緑）──', '', 'section'],
    ['理論売値', '=ROUND(B15/(1-B6-B14),0)', 'result', '¥#,##0', '=原価÷(1-目標粗利率-手数料合計)'],
    ['おすすめ売値(10円丸め)', '=ROUND(B18,-1)', 'result', '¥#,##0', '理論売値を10円単位に丸め'],
    ['おすすめ売値(100円丸め)', '=ROUND(B18,-2)', 'result', '¥#,##0', '理論売値を100円単位に丸め'],
    ['推奨うわのせ(10円丸め)', '=B19-B15', 'result', '¥#,##0', '=おすすめ売値(10円)-原価'],
    ['推奨うわのせ(100円丸め)', '=B20-B15', 'result', '¥#,##0', '=おすすめ売値(100円)-原価'],
    ...
    ['実際売値', '=B15+B25', 'actual', '¥#,##0', '=原価(B15)+うわのせ(B25)'],
    ['実質粗利額', '=B26-B26*B14-B15', 'actual', '¥#,##0', '=売値-売値×手数料合計-原価'],
    ['実質粗利率', '=(B27)/B26', 'actual', '0%', '20%以上が目安'],
    ['判定', '=IF(B28<0.2,"⚠ 20%割れ 注意","OK")', 'actual', '@', '微調整後の粗利率で判定'],
  ];
...
function 計算機_buildSheet_(ss, name, rows) {
  ...
  rows.forEach(function (r, idx) {
    ...
    if (note) sh.getRange(row, 3).setValue(note).setFontColor('#666666').setFontSize(10);

...
const mapM = ctx.yt_ヘッダー名マップ_([
  ['発番発行','登録状況','商品コードステータス','親コード','タイトル','作者','','','','','売価','','','','','','','','','','配送パターン','','ISBN','発売日'],
]);
const row = ['', true, '生成済み', 'TWS0152-CM-04', '台湾版まんが(特装版)『…』', '原作:…', '','','','','¥2,480','','','','','','','','','','佐川','','9784000000000','2026/06/18'];
...
// 値段: ソースの「売価」列から取得（Yahoo①商品入力シートの2行目ヘッダー名は「値段」）
eq('値段(売価から)', mapped['値段'], '¥2,480');
// 売価ヘッダーが無いタブでは空欄
const mapNoPrice = ctx.yt_ヘッダー名マップ_([['発番発行','親コード','タイトル']]);
const mappedNoPrice = ctx.yt_行から送信値_(['', 'TWS0001-CM-01', 'X'], mapNoPrice);
eq('値段(売価なしタブは空)', mappedNoPrice['値段'], '');

...
function onOpen() {
  ...
  menu
    ...
    .addItem('♻ 解放実行（欠番管理の☑を反映）', '台湾_欠番解放実行')
    ...
    .addSubMenu(
      ui.createMenu('台湾まんが')
        ...
        .addItem('🈁 選択行の日本語タイトルを照会（サイト検索）', '台湾_日本語タイトルを照会')
        .addItem('⑥ プルダウン更新', '台湾まんが_プルダウン更新')
        .addSeparator()
        .addSubMenu(
          ui.createMenu('🔗 重複の統合（3ステップ）')
            .addItem('① 🔎 検出（ドライラン＝色付け）', '台湾書籍系_重複作品_検出ドライラン')
            .addItem('② 🎨 検出の色をクリア', '台湾書籍系_重複作品_色をクリア')
            .addItem('③ 🔗 統合を実行', '台湾書籍系_重複作品_統合実行')
        )
        .addSubMenu(
          ui.createMenu('🩺 Works点検・修復')
            .addItem('健全性チェック（まず実行）', '台湾書籍系_Works健全性チェック_')
            .addItem('WorksKeyに媒体コード付与', '台湾書籍系_WorksKey媒体付与_削除なし')
            .addItem('登録済み商品からWorksを復元（全行・同じ作品IDで作成＋媒体付与）', '台湾書籍系_登録済み商品からWorksを復元_')
            .addItem('登録済み商品からWorksを復元（チェック行だけ）', '台湾書籍系_登録済み商品からWorksを復元_チェック行のみ_')
            .addItem('作品ID・SKUを確定コードに揃える（登録済み行）', '台湾書籍系_作品IDを確定コードに揃える_')
            .addItem('Works最新巻を再計算', '台湾書籍系_Works最新巻を再計算メニュー_')
            .addSeparator()
            .addItem('⏰ 定期メンテを設定（毎日3時・自動修復＋記録）', '台湾書籍系_定期メンテを設定_')
            .addItem('⏰ 定期メンテを解除', '台湾書籍系_定期メンテを解除_')
            .addSeparator()
            .addItem('🐞 媒体不明の診断(0009/K-9)', '台湾書籍系_媒体不明を診断メニュー_')
            .addItem('🔒 危険操作は停止中', '台湾書籍系_Works危険操作メニュー説明')
        )
    )
    ...
    .addSubMenu(
      ui.createMenu('台湾雑誌')
        // 雑誌は確定発行の概念がなく再計算でコード生成される。旧「① 確定発行」は関数未実装で
        // クリック時エラーだったため、実在する全行再計算に差し替え。
        .addItem('① 全行再計算（コード生成）', '台湾雑誌_全行再計算')
 ...
 * ============================================================ */
function 台湾まんが_確定発行() {
  // ★ _kyoutuu（媒体無視・全再正規化で CM/NV を破壊）を使わず、媒体対応のローカル確定発行に差し替え。
  台湾書籍系_ローカル確定発行_('台湾まんが', 設定_台湾まんが);
}
...
function 台湾書籍その他_確定発行() {
  // ★ _kyoutuu（媒体無視・全再正規化で CM/NV を破壊）を使わず、媒体対応のローカル確定発行に差し替え。
  台湾書籍系_ローカル確定発行_('台湾書籍その他', 設定_台湾書籍その他);
}
 ...
 * ============================================================ */
/*
 * 以下の _kyoutuu書籍*_ 5関数は、旧ライブラリ(_kyoutuu＝媒体無視のWorksKey方式)による
 * Works破壊操作（全削除・媒体なしキーへの再正規化・統合・ID振り直し・孤立削除）だった。
 * メニューからは既に停止済みだが、関数本体も無害化して誤実行・誤配線を封じる。
 * 重複の整理は、媒体対応の「重複の統合（3ステップ）」を使うこと。
 */
function _kyoutuu書籍Works初期化_(cfg) {
  台湾書籍系_Works危険操作を停止_('Works初期化（旧ライブラリ）');
}

function _kyoutuu書籍WorksKey再正規化_(cfg) {
  台湾書籍系_Works危険操作を停止_('WorksKey再正規化（旧ライブラリ・媒体コードを剥がすため封印）');
}

function _kyoutuu書籍重複統合_(cfg) {
  台湾書籍系_Works危険操作を停止_('Works重複統合（旧ライブラリ）。重複整理は媒体対応の「重複の統合（3ステップ）」を使用');
}

function _kyoutuu書籍ID振り直し_(cfg) {
  台湾書籍系_Works危険操作を停止_('Works ID振り直し（旧ライブラリ・永久IDを変更するため封印）');
}

function _kyoutuu書籍孤立削除_(cfg) {
  台湾書籍系_Works危険操作を停止_('Works孤立エントリー削除（旧ライブラリ）');
}

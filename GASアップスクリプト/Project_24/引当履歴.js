...
function 到着済を在庫反映済みへ本体_(){
  ...
  if(箱余り.length && typeof Yahoo変更_対象行_==='function' && typeof Yahoo在庫変更を出力本体_==='function'){
    ...
    // Yahooへ出せないコード(親コードを逆引きできない)はその場で計算する。保存済み記録に頼ると
    // 古い記録(要確認コードを持たない)を「要確認0件」と誤読し、元の不具合が再発する(2026-07-24レビュー)
    let 要確認コード=[];
  ...
  const moveLines=['EMS番号 / 商品コード / Yahooへ移す数量'].concat(
    ...
    .concat(除外行.length?['','■ Yahooへ入れないので移動記録しません(現物は日本にあります)',
      ...
      .concat(未出品行.length?['　 未出品の分は「日本在庫の戻り行」へ持ち越すので、Yahoo出品後にCSVを作れます。']:[])

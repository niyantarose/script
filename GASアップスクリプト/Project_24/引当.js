...
function 引当実行_本体_(){
  ...
  try{ 引当履歴_今回到着分を記録_(true); }catch(e){} // EMSリストの到着済+注文番号を、二重記録なしで履歴に残す
  ...
  const 出荷済内訳=[]; // 今回の箱を食った出荷済み(台帳)の一覧。キャンセル誤検知にすぐ気づけるよう完了ダイアログに出す
  出荷済行.forEach(l=>{ if(!出荷済消費OK_(l)) return; const k=keyInStock(l); if(k!=null){ stock[k]=Math.max(0,(stock[k]||0)-l.qty); 出荷済内訳.push('・'+l.ban+' '+(l.code||l.sku)+' x'+l.qty); } });
  ...
  // 完了サマリは大きなダイアログで表示(トーストは小さく末尾が切れるため)
  {
    ...
    const 突合行= 突合宙.length
      ? '⚠️ 箱と合わない割当 '+突合宙.length+'件（二重引当/誤入荷日/台帳重複の疑い）:\n'
        +突合宙.slice(0,20).join('\n')+(突合宙.length>20? '\n…他'+(突合宙.length-20)+'件':'')
        +'\n→ 🔎入荷日の整合チェック / 🔎引当診断で確認'
      : '✅ OK『'+突合式+'』';
    const 誤行= 誤入荷>0? '⚠️ 誤入荷日 '+誤入荷+'件（その到着日に箱が無い入荷日）→ 🔎整合チェック→🧹で掃除'
      : (誤入荷===0? '✅ 誤入荷日なし' : '（誤入荷日チェックは実行できませんでした）');
    const 出荷済表示= 出荷済内訳.length
      ? '\n📦 出荷済み消込の内訳(台帳): '+出荷済内訳.length+'件\n'+出荷済内訳.slice(0,10).join('\n')
        +(出荷済内訳.length>10? '\n…他'+(出荷済内訳.length-10)+'件':'')
        +'\n（本当に発送済みか確認。キャンセルだった注文は消込台帳の状態を「キャンセル」に直す）'
      : '';
    ui.alert((突合宙.length||誤入荷>0)? '⚠️ 引当完了（要確認あり）' : '✅ 引当完了（整合OK）',
      '■ 突合せ（引当＋在庫 と EMS到着済）\n'+突合行+'\n'+誤行+出荷済表示
      +'\n\n■ 注文の分類\n出荷可能 '+ship+' ／ 出荷GO未入金 '+keep+' ／ 希望日待ち '+hold+' ／ 部分在庫 '+part+' ／ 引当待ち '+(ord-ship-keep-hold-part)+'（うち入金待ち '+mp+'）'
      +'\n\n■ 処理内容\n入荷日自動記入 '+自動入荷+'件 ／ P列確定 '+確定行数+'行 ／ 出荷済み消込 '+出荷済行.length+'件'

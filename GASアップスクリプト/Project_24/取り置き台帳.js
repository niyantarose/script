const TORIOKI_CFG = Object.freeze({
  ...
  台帳HDR:['取置ID','状態','受注番号','商品コード','SKU','取り置き数量','取置元種別','元EMS番号','元EMS商品コード','元取置ID','登録日時','更新日時','戻し処理結果','終了理由・メモ','引当段階','EMS到着予定日','現物確認日時','現物確認メモ','供給控除EMS'],
...
function 取り置き_表を読む_(sheetName, headers){
  ...
  // 既存の14列台帳は読込を許可し、保存時にだけ末尾の三段階列を展開する。

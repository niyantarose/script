const TORIOKI_CFG = Object.freeze({
  ...
  // 台帳確保(文章)を確保済み/不足(数字)へ置換(2026-07-23ユーザー要望: 移行シートと同じ見え方)
  初期HDR:['取置ID','受注番号','氏名','商品コード','SKU','注文数量','確保済み','不足','現在の状態','受注ステータス','旧入荷日','旧EMS','棚確認','現物取り置き数量','メモ','判定','要対応','処理'],
...
function 取り置き初期登録を作成本体_(options){
  ...
  candidates.forEach(c=>{
    const secured=(Number(c.台帳確保数)||0)+(現物登録[取り置き_行キー_(c)]||0);
    c.確保済み=secured;

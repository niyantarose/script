const TORIOKI_CFG = Object.freeze({
  ...
  初期HDR:['取置ID','受注番号','氏名','商品コード','SKU','注文数量','確保済み','不足','空き在庫','確保内訳','受注ステータス','追加数量','マイナス数量','棚確認','メモ','判定','要対応','処理'],
...
function 取り置き初期登録を作成本体_(options){
  ...
  // 空き在庫(誰にも引き当たっていない現物)を商品コードで引いて出す。不足があっても
  // 空きがあれば引き当てられる/空き0なら本当に待ち、が一目で分かる(2026-07-24)
  {
    const 空き=取り置き_空き在庫マップ_();
    candidates.forEach(c=>{
      const code=normCode_(取り置き_商品コード_(c.SKU,c.商品コード));
      const n=空き[code]||空き[normCode_(c.SKU)]||空き[normCode_(c.商品コード)]||0;
      c.空き在庫=n||'';
    });
  }
  ...
  // 失敗を黙って捨てると「受注明細が更新されない」原因が分からなくなるので必ず知らせる(2026-07-24)
  try{ if(typeof 受注明細_確保列を書く_==='function') 受注明細_確保列を書く_(candidates); }
  catch(e){ try{ ss.toast('受注明細の確保済み/不足/確保内訳を更新できませんでした: '+e.message,'⚠️受注明細',10); }catch(e2){} }
...
function 取り置き登録を反映本体_(){
  ...
  // ④/②の自動確保だけを外す入力(棚登録0の行へマイナス)も「適用できる入力」に数える(2026-07-24)
  const 適用あり=c.取り置き行>0||(c.解除||0)>0||(c.自動解除数量||0)>0||c.棚へ戻した>0||c.現物なし>0;
  ...
  SpreadsheetApp.getActive().toast(
    ...
    '取り置き登録',8);
  // 台帳が変わると 引当状況一覧・日本在庫・受注明細の色/EMS番号・出荷可能などは古くなる。
  // これらを作れるのは②だけなので、その場で流すかを聞く(2026-07-24「全部のシートを最新に」)
  const 続けて=ui.alert('②引当を実行して全シートを最新にしますか？',
    '取り置き台帳は更新しました。ただし引当状況一覧・日本在庫・受注明細のEMS番号や色・\n'
    +'出荷可能/部分在庫などの一覧は②が作り直すまで前のままです。\n\n'
    +'「はい」で今すぐ②を実行します(30秒ほど)。「いいえ」なら後で②を回してください。',ui.ButtonSet.YES_NO);
  if(続けて===ui.Button.YES){
    try{ 引当実行_本体_({}); }
    catch(e){ ui.alert('②の実行に失敗しました',e.message+'\n(取り置き台帳の反映は完了しています)',ui.ButtonSet.OK); }
  }
}
...
// 商品コード→空き在庫(誰にも引き当たっていない現物)の一覧。日本在庫シートの
// 「到着済」(箱の余り)と「戻り」(棚へ戻った現物)を足す。在庫対象外は数えない。
// ②が最後に計算した時点の数字なので、反映直後は次の②まで古いことがある(見出しで明示)
function 取り置き_空き在庫マップ_(){
  const out={};
  try{
    const sh=SpreadsheetApp.getActive().getSheetByName(HIKIATE_CFG.純在庫);
    if(!sh || sh.getLastRow()<3) return out;
    sh.getRange(3,1,sh.getLastRow()-2,5).getDisplayValues().forEach(r=>{
      const 状態=String(r[0]||'').trim();
      if(状態!=='到着済' && 状態!=='戻り') return;
      const code=normCode_(r[2]), qty=Number(String(r[3]||'').replace(/[^0-9.-]/g,''))||0;

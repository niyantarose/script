...
// 完了通知でGASの実行を停止しない。ui.alertは閉じるまで実行時間を消費し、6分上限に達する。
function 取り置き_更新完了を通知_(ss,message,silent){
  if(silent) return;
  ss.toast(String(message||''),'取り置き登録を更新しました',10);
}
...
function 取り置き初期登録を作成本体_(options){
  ...
  candidates.forEach(c=>{
    ...
    // 受注明細の現在表示用。集計候補と同じ有効台帳行から実EMSだけを取り出す(台帳へは保存しない)。
  ...
  const 完了メッセージ='表示モード: '+表示モード+'（全'+全注文数+'注文中 '+表示注文数+'注文を表示。切替はメニュー👀）\n'
    ...
    +(入力済? '\n入力済みの数量'+入力済+'行は引き継いでいます。':'')
    +要引当メモ;
  取り置き_更新完了を通知_(ss,完了メッセージ,silent);
  return summary;
}

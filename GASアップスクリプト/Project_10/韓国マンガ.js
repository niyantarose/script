...
const 設定_韓国マンガ = {
  ...
  列名: {
    発行チェック:     '発番発行',       // 韓国マンガにはチェックボックス列を追加予定
    商品コード:       '管理コード',
    タイトル:         'タイトル(自動)', // 自動生成タイトル列（必要なら追加）
    作者:             '著者',
    日本語タイトル:   '商品名(日本語)',
    原題:             '商品名(原題)',
    形態:             '版種名',
 ...
 * 韓国マンガ メニューラッパー
 * ============================================================ */
function 韓国マンガ_確定発行()       { 確定発行を実行(設定_韓国マンガ); }
function 韓国マンガ_削除()           { 削除を実行(設定_韓国マンガ); }
function 韓国マンガ_一括更新()       { 一括更新を実行(設定_韓国マンガ); }
function 韓国マンガ_プルダウン更新() { プルダウン更新を実行(設定_韓国マンガ); }
...
function 韓国マンガ_WorksKey再正規化() {
  ...
  try {
    const r = WorksKey再正規化を実行(作品シート, 設定_韓国マンガ);
    ui.alert(`✅ 完了\nキー更新: ${r.キー更新数}\n重複統合: ${r.統合数}\n削除: ${r.削除行数}`);
...
function 韓国マンガ_重複統合() {
  ...
  const lock = LockService.getDocumentLock(); lock.waitLock(30000);
  try {
    const 重複リスト = Works重複を検出(作品シート, 設定_韓国マンガ);
...
function 韓国マンガ_ID振り直し() {
  ...
  const lock = LockService.getDocumentLock(); lock.waitLock(30000);
  try {
    const r = WorksID振り直しを実行(作品シート, 設定_韓国マンガ);
    ui.alert(`✅ ID振り直し完了\n変更: ${r.変更数}件`);
  } finally { lock.releaseLock(); }
}
...
function 韓国マンガ_孤立削除() {
  ...
  try {
    ...
    const r = Works孤立エントリーを削除(作品シート, マスターシート, 列マップ, 設定_韓国マンガ);

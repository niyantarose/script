...
const ALADIN_COLOR_MAP = {
  '韓国書籍': {
    ...
    output:  ['商品名(原題)', '著者', '出版社', '発売日', '原価', 'メイン画像URL', '追加画像URL', '商品説明', 'カテゴリ', 'アラジン商品ID'],
  },
  '韓国マンガ': {
    trigger: ['ISBN', 'アラジンURL'],
    output:  ['商品名(原題)', '著者', '出版社', '発売日', '原価', 'メイン画像URL', '追加画像URL', '商品説明', 'カテゴリ', 'アラジン商品ID'],
  ...
  '韓国音楽映像': {
    trigger: ['アラジンURL'],
    output:  ['商品名(タイトル)', '商品名(原題)', 'アーティスト名', '事務所レーベル', '発売日', '原価', 'メイン画像URL', '追加画像URL', '商品説明', 'カテゴリ', 'アラジン商品ID', 'JANコード'],
  },
  '韓国グッズ': {
    trigger: ['購入URL'],
    output:  ['商品名（原題）', '発売日', '原価', 'メイン画像URL', '追加画像URL', '商品説明', 'アラジン商品ID'],
  },
};
...
function アラジントリガー列に色付け() {
  ...
  for (const [shName, colorDef] of Object.entries(ALADIN_COLOR_MAP)) {
    const sh = ss.getSheetByName(shName);
    if (!sh) { 結果.push(`⚠ ${shName}：シートが見つかりません`); continue; }

    const lastCol = sh.getLastColumn();
    if (lastCol < 1) { 結果.push(`⚠ ${shName}：列がありません`); continue; }

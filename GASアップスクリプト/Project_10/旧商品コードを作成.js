...
function Edit(e) {
  if (!e || !e.range) {
    if (typeof onEditログ_ === 'function') onEditログ_('旧Edit:no_event', { sheet: '台湾まんが' });
    return;
  }

  const sh = e.range.getSheet();
  if (typeof onEditログ_ === 'function') {
    onEditログ_('旧Edit:entry', {
      sheet: sh.getName(),
  ...
  if (sh.getName() !== 設定.マスターシート名) {
    if (typeof onEditログ_ === 'function') onEditログ_('旧Edit:delegate_to_common', { sheet: sh.getName(), target: 設定.マスターシート名 });
    if (typeof メインonEdit === 'function') メインonEdit(e);
    return;
  }
...
function 作品データを更新_予約込み(作品シート, 作品データ, 予約巻数マップ) {
  ...
  for (const [key, 巻数リスト] of Object.entries(予約巻数マップ)) {
    ...
    予約更新.push({ row: 行番号, values: [新最新巻, now] });
  }
  if (予約更新.length > 0) {
...
function シート設定を取得(シート名) {
  const 台湾書籍設定 = (typeof 設定_台湾書籍 !== 'undefined') ? 設定_台湾書籍 : 設定;
  const cfgForSheet = (cfg, name, columnOverrides) => {
    const next = Object.assign({}, cfg, { マスターシート名: name });
    next.列名 = Object.assign({}, cfg.列名, columnOverrides || {});
    return next;
  };
  const マップ = {
    '台湾_書籍（コミック/小説/設定集）': 台湾書籍設定,
    '台湾まんが': cfgForSheet(台湾書籍設定, '台湾まんが', { 商品コード: '親コード' }),
    '韓国マンガ': (typeof 設定_韓国マンガ !== 'undefined') ? 設定_韓国マンガ : 設定,
  };

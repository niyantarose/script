 ...
 * ============================================================ */
function onEditログ_(label, payload) {
...
function メインonEdit(e) {  // onEdit → メインonEdit に変更
  if (!e || !e.range) {
    onEditログ_('メインonEdit:no_event', { sheet: '台湾まんが' });
    return;
  }
  const sh = e.range.getSheet();
  const shName = sh.getName();
  onEditログ_('メインonEdit:start', {
    sheet: shName,
  ...
  if (!cfg) {
    onEditログ_('メインonEdit:no_cfg', { sheet: shName });
    return;
  }
  ...
  if (開始行 + 行数 - 1 < 2) {
    onEditログ_('メインonEdit:header_skip', { sheet: shName, row: 開始行, rows: 行数 });
    return;
  }
  ...
  try {
    if (!lock.tryLock(5000)) {
      onEditログ_('メインonEdit:lock_skip', { sheet: shName, a1: e.range.getA1Notation() });
      return;
    }
  } catch (err) {
    onEditログ_('メインonEdit:lock_error', { sheet: shName, error: String(err) });
    return;
  }

  try {
    if (自己更新中か_()) {
      onEditログ_('メインonEdit:self_update_skip', { sheet: shName, a1: e.range.getA1Notation() });
      return;
    }
    ...
    onEditログ_('メインonEdit:watch_check', {
      sheet: shName,
      editedStartColumn: 編集開始列,
      editedEndColumn: 編集終了列,
      watchedColumns: 監視列番号,
      codeHeader: cfg.列名 && cfg.列名.商品コード,
      codeColumn: 列マップ[cfg.列名 && cfg.列名.商品コード],
      volumeHeader: cfg.列名 && cfg.列名.単巻数,
      volumeColumn: 列マップ[cfg.列名 && cfg.列名.単巻数],
      matched: 対象列が含まれる
    });
    if (!対象列が含まれる) return;
    自己更新を開始_();
...
function onEdit(e) {
  onEditログ_('onEdit:entry', {
    sheet: e && e.range ? e.range.getSheet().getName() : '台湾まんが',
    a1: e && e.range ? e.range.getA1Notation() : ''
  });
  メインonEdit(e);
}

function 商品コードに巻数を反映_(sku, 単巻数, セット開始, セット終了) {
  const baseSku = String(sku || '').trim();
  if (!baseSku) return baseSku;

  const sf = String(セット開始 || '').trim();
  const st = String(セット終了 || '').trim();
  const base = baseSku.replace(/-(?:\d{2}|\d{4}|SET)$/i, '');

  if (sf && st) {
    const diff = parseInt(st, 10) - parseInt(sf, 10);
    if (!isNaN(diff) && diff >= 2) return base + '-SET';
...
function onEdit処理を実行(e, sh, cfg, 列マップ, 開始行, 行数) {
  ...
  const cn = cfg.列名;
  onEditログ_('onEdit処理:start', {
    sheet: cfg.マスターシート名,
    startRow: 開始行,
    rows: 行数,
    lastColumn: 最終列,
    codeHeader: cn.商品コード,
    codeColumn: 列マップ[cn.商品コード],
    volumeHeader: cn.単巻数,
    volumeColumn: 列マップ[cn.単巻数],
    titleHeader: cn.タイトル,
    titleColumn: 列マップ[cn.タイトル]
  });
  ...
  for (let i = 0; i < 行数; i++) {
    ...
    if (!日本語タイトル || !作者) {
      onEditログ_('onEdit処理:row_skip_missing', {
        sheet: cfg.マスターシート名,
        row: 行番号,
        japaneseTitle: 日本語タイトル,
        author: 作者,
        oldCode: 取得生(cn.商品コード),
        volume: 単巻数
      });
    ...
    const SKU = 商品コードに巻数を反映_(
      SKUを段階生成(言語コード, 形態コード, 作品ID, カテゴリコード, 単巻数, セット開始, セット終了),
      単巻数,
      セット開始,
      セット終了
    );
    onEditログ_('onEdit処理:row_result', {
      sheet: cfg.マスターシート名,
      row: 行番号,
      japaneseTitle: 日本語タイトル,
      author: 作者,
  ...
  onEditログ_('onEdit処理:write_done', {
    sheet: cfg.マスターシート名,
    startRow: 開始行,
    rows: 行数,
    codeHeader: cn.商品コード,
    codeColumn: 列マップ[cn.商品コード],
    outputCodes: 出力SKU.map(r => r[0]).slice(0, 10),
    statusValues: 出力ステータス.map(r => r[0]).slice(0, 10)
  });
...
function 作品データを更新_予約込み(作品シート, 作品データ, 予約巻数マップ, cfg) {
  ...
  for (const [key, 巻数リスト] of Object.entries(予約巻数マップ)) {
    ...
    予約更新.push({
      row: 行番号,
      values: [Math.max(今回最大, 既存), now],
    });
  }
  if (予約更新.length > 0) {
    予約更新.sort((a, b) => a.row - b.row);
    let startRow = 予約更新[0].row;
    let rows = [予約更新[0].values];
    for (let i = 1; i < 予約更新.length; i++) {
      const item = 予約更新[i];
      if (item.row === startRow + rows.length) {
        rows.push(item.values);
        continue;
      }
      作品シート.getRange(startRow, 9, rows.length, 2).setValues(rows);
      startRow = item.row;
      rows = [item.values];
    }
    作品シート.getRange(startRow, 9, rows.length, 2).setValues(rows);
...
function 確定発行を実行(cfg) {
  ...
  try {
    ...
    for (let i = 0; i < 全データ.length; i++) {
      ...
      const SKU = 商品コードに巻数を反映_(
        SKUを生成(言語コード, 形態コード, 作品ID, カテゴリコード, 取得生(cn.単巻数), 取得生(cn.セット開始), 取得生(cn.セット終了)),
        取得生(cn.単巻数),
        取得生(cn.セット開始),
        取得生(cn.セット終了)
      );
...
function 一括更新を実行(cfg) {
  ...
  try {
    ...
    for (let i = 0; i < 全データ.length; i++) {
      ...
      const SKU = 商品コードに巻数を反映_(
        SKUを段階生成(言語コード, 形態コード, 作品ID, カテゴリコード, 取得生(cn.単巻数), 取得生(cn.セット開始), 取得生(cn.セット終了)),
        取得生(cn.単巻数),
        取得生(cn.セット開始),
        取得生(cn.セット終了)
      );
      ...
      outステータス.push([(日本語タイトル && 作者 && 言語 && カテゴリ) ? '生成済み' : '入力中...']);

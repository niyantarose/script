...
async function initPopup() {
  try {
    ...
    renderList();
    await refreshGasSyncStatus({ updateUi: false });
    updateUI();
...
async function handleAddCurrentPage() {
  ...
  try {
    ...
    if (!(typeof isIgnorableMangaUpdatesError === 'function' && isIgnorableMangaUpdatesError(error))) {
...
async function handleDownloadImagesOnly() {
  ...
  const imageResult = await downloadProductImages(products, { manualSave: manualSaveImages });
  setStatus(
    `✅ 画像:${imageResult.success}/${imageResult.total}件保存 / フォルダ:${imageResult.folders.length}` +
      (manualSaveImages ? ' / 手動保存ダイアログ' : ' / 自動連番モード') +
...
async function handleDownloadCsvAndImages() {
  ...
  if (books.length > 0) {
    ...
    const ok = await downloadCsvFile([...baseHeaders, ...bookHeaders], rows, buildCsvDownloadPath(`bokukuro_${dateStr}_books.csv`));
    if (ok) exported += 1;
  }

  if (magazines.length > 0) {
    const rows = magazines.map(buildMagazineCsvRow);
    const ok = await downloadCsvFile([...baseHeaders, ...magazineHeaders], rows, buildCsvDownloadPath(`bokukuro_${dateStr}_magazines.csv`));
    if (ok) exported += 1;
  }

  if (goods.length > 0) {
    const rows = goods.map(buildCsvBaseRow);
    const ok = await downloadCsvFile(baseHeaders, rows, buildCsvDownloadPath(`bokukuro_${dateStr}_goods.csv`));
    if (ok) exported += 1;
  }

  const imageResult = await downloadProductImages(products, { manualSave: manualSaveImages });
  setStatus(
    `✅ CSV:${exported}ファイル / 画像:${imageResult.success}/${imageResult.total}件保存 / フォルダ:${imageResult.folders.length}` +
      (manualSaveImages ? ' / 手動保存ダイアログ' : ' / 自動連番モード') +
      (imageResult.failed > 0 ? `（失敗:${imageResult.failed}）` : ''),
    imageResult.failed > 0 ? 'error' : 'success'
  );

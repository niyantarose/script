...
function extractTaiwanVolumeNumber_(text) {
  ...
  const groups = [
    ...
    // 中: 巻数の直後に版種/特典ブラケットが続く形（例: 摸布想自己賺罐罐3【簽名特別版】/ …2（隨書附贈…））
    //     直前が英数字の場合は作品名の一部（例: 1Q84（上））なので拾わない。
    [
      /(?:^|[^\dA-Za-z])(\d{1,3})\s*[【（(\[]\s*[^】）)\]]{0,40}?(?:首刷|初版|初回|限定|特[裝装]|特別|特别|簽名|签名|特典|附贈|附赠|加贈|加赠|贈品|赠品|隨書|随书|通常|完)/u,
      /(?:vol(?:ume)?\.?\s*)(\d{1,3})/iu, /[)\]】》」』]\s*(\d{1,3})\s*$/u, /\((\d{1,3})\)\s*$/u, /[(（](\d{1,3})[)）]/u
    ],
    // 緩: 版種キーワード直前の数字 / 末尾の数字（タイトル途中の数字は拾わない）
    [
      /(?:^|[^\d])(\d{1,3})\s*[(（]?\s*[)）]?\s*(?:首刷|初版|初回|限定|特[裝装]|通常|完)/u,
      /(?:^|[^\d])(\d{1,3})\s*$/u
    ],
  ];

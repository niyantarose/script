...
const scoreEdgeDistance = edge => {
  ...
  const cleanupOriginalTitleOnce = value => value
    ...
    .replace(/\s*【[^】]*(?:首刷|初回|初版|限定|特裝|特装|特別|特别|通常|普通|獨家|独家|博客來獨家|博客来独家|特典|附贈|加贈|贈品|赠品|附錄|附录|隨書|随书)[^】]*】\s*/gu, ' ')
    .replace(/\s*\([^)]*(?:首刷|初回|初版|限定|特裝|特装|特別|特别|通常|普通|獨家|独家|博客來獨家|博客来独家|特典|附贈|加贈|贈品|赠品|附錄|附录|隨書|随书)[^)]*\)\s*/gu, ' ')
    .replace(/\s*\[[^\]]*(?:首刷|初回|初版|限定|特裝|特装|特別|特别|通常|普通|獨家|独家|博客來獨家|博客来独家|特典|附贈|加贈|贈品|赠品|附錄|附录|隨書|随书)[^\]]*\]\s*/gu, ' ')
    .replace(/\s*（[^）]*(?:首刷|初回|初版|限定|特裝|特装|特別|特别|通常|普通|獨家|独家|博客來獨家|博客来独家|特典|附贈|加贈|贈品|赠品|附錄|附录|隨書|随书)[^）]*）\s*/gu, ' ')
    ...
    .replace(/\s*(?:首刷(?:限定)?|初回(?:限定)?|初版(?:限定)?|限定|特裝|特装|特別|特别|通常|普通|獨家|独家|特典)(?:版)?\s*$/u, '')
    // N+M 合本表記（例: 3+4 / 1＋2套書）は丸ごと除去。このルールが無いと下の末尾数字ルールが
    // 片割れの「4」だけ食べて「今生我來當家主3+」のようなゴミが残り、日本語タイトル照会が外れる。
    .replace(/[0-9０-９]{1,3}(?:\s*[+＋]\s*[0-9０-９]{1,3})+(?:\s*(?:小說|小説|小说|漫畫|漫画|コミック|單行本|单行本|套書|套书))?(?:\s*(?:限量|限定))?\s*$/iu, '')
    // 末尾の単独「全/完」（例: 后宮的Ω王子 雪花之章 全）。「大全」等を壊さないよう直前に空白必須
    .replace(/[\s　]+(?:全|完)\s*$/u, '')

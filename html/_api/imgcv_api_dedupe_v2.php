...
// ルート階層でも debug 階層でも受け取れるように改良！
$force_commit = !empty($req['force_commit']) || !empty($req['debug']['force_commit']);
...
$force   = !empty($req['force']); // ★追加
  need(is_array($codes), 'items must be array');
  need(is_array($desired), 'desired_images must be object');

// ★ここに追加
need(count($codes) > 0,  'items is empty');
need(!empty($desired),   'desired_images is empty');
...
// ★ main画像（AAA.jpg）が無い場合は事故防止で弾く
    if ($main === '') {
      $failed++;
      $details[]=['ok'=>false,'item_code'=>$code,'status'=>'NO_MAIN'];
      continue;
    }
...
if (!$force && $prev !== null && hash_equals($prev, $dh)) {
      $skipped++;
...
$rp = yahoo_reserve_publish($seller, $token, $targetCodes, $desired);

      if (empty($rp['ok'])) {
        respond(['ok'=>false,'error'=>'Yahoo API failed','reserve_publish'=>$rp], 502);
      }

      apply_reserve_publish_result(
        $rp,
        $pending,
        fn(string $code, string $dh) => set_submit_hash($seller, $code, $dh),
        $out
      );
    }
  }

...
同一 product_code で inventory_type='即納' の行が複数存在しないか確認する。

```sql
SELECT product_code, COUNT(*) FROM inventory WHERE inventory_type='即納' GROUP BY product_code HAVING COUNT(*)>1;
```

Expected: 0件。複数あればシード前に手動で1行に統合しておく(重複したままシードすると台帳の期首残高が商品ごとに二重計上される)。

- [ ] **Step 1: ローカルDBでシード実行**

Run: `python scripts/seed_ledger_opening.py`
Expected: `期首残高: {'seeded': N, 'skipped': M} / 出庫バックフィル: {'recorded': R, 'checked': C} / YahooモールSKU: {'created': X, 'skipped': Y}` (N,X,R,C は環境のデータ量に依存)

**注意**: 即納 quantity は旧システムで入荷加算のみ行われ出荷時の減算がされていないため、累積値になっている可能性が高い。シード後、主要商品について実棚(実際の手元在庫)と台帳在庫を突合し、差異があれば台帳画面の「調整」機能で訂正してから運用を開始すること。

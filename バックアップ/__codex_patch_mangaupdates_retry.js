...
if (!shared.includes('const MANGA_UPDATES_API_BASE =')) {
  const marker = 'function normalizeBooksImageUrl(rawUrl) {';
  if (!shared.includes(marker)) throw new Error('popup.shared marker not found');
  const helperBlock = [
  ...
  shared = shared.replace(marker, helperBlock + marker);
...
if (!popup.includes('enrichProductWithMangaUpdatesJapaneseTitle')) {

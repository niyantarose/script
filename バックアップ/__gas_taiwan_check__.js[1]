...
function doPost(e) {
  ...
  try {
    ...
    try {
      ...
      const appended = results.filter(function(r) { return r && r.ok; }).length;
...
function appendItems_(ss, items) {
  ...
  Object.keys(groupedBySheet).forEach(sheetName => {
    ...
    entries.forEach(entry => {
      const duplicateKeys = buildDuplicateKeysForRowData_(extractRowData_(entry.item), duplicateKeyColumns);
      ...
      rows.push(buildRow_(context, entry.item));
    });

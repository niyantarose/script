...
function resolveJapaneseTitleLookupFields_(item) {
  ...
  if (!rowData || itemType === 'magazine') return [];
  ...
  const rawCategory = String(rowData['カテゴリ'] || '').trim();
  const category = rawCategory ? normalizeDictionaryCategory_(rawCategory) : '書籍';
  if (category !== '書籍') return [];
...
function findTitleFromDictionary_(language, category, originalTitle, author) {
  ...
  const titleKey = normalizeDictionaryTitleKey_(originalTitle);
  if (!normalizedLanguage || !normalizedCategory || !titleKey) return '';

  const dictionary = getJapaneseTitleDictionaryIndex_();
  const candidates = dictionary.byKey[buildJapaneseTitleDictionaryKey_(normalizedLanguage, normalizedCategory, titleKey)] || [];
...
function convertJapaneseTitle_(item) {
  ...
  try {
    ...
    for (let i = 0; i < lookupFieldsList.length; i += 1) {
      ...
      japaneseTitle = findTitleFromDictionary_(fields.language, fields.category, fields.originalTitle, fields.author);
    ...
    const rowData = Object.assign({}, extractRowData_(item), {
      '日本語タイトル': japaneseTitle,
    });

    if (String(item.itemType || '') === 'goods') {
    ...
    Logger.log('Japanese title dictionary conversion failed: ' + error.message);
    return item;
  }
}
function testFindTitleFromDictionary_() {
  const result = findTitleFromDictionary_('台湾', '書籍', '鄰居是公會成員', 'HoneyTrap');
...
function isStrongMangaUpdatesMatch_(query, searchResult, detail) {
  const queryKey = normalizeMangaUpdatesTitleKey_(query);
  ...
  return candidates.some(function(candidate) {
    return normalizeMangaUpdatesTitleKey_(candidate) === queryKey;
  });
}
